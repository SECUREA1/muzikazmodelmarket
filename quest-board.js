(function(){
  ...
  async function render(){
    ...
    $('#reward-summary').value=`${pts} points · ${bal} Mixer Tokens · earned today ${walletLedger.filter(x=>x.amount>0&&x.createdAt.startsWith(today)).reduce((a,x)=>a+x.amount,0)} · pending ${quests.reduce((a,q)=>a+(((p[key(q)]||0)>=q.goal&&!c[key(q)])?(q.mixerTokenReward||q.reward):0),0)}`;
    $('#daily-streak').textContent=`${d.streak||0} day streak`;
    $('#daily-drop').textContent=`+${dailyDrop.points} pts · +${dailyDrop.tokens} Mixer Tokens`;
    ...
    $('#quests').innerHTML=quests.map(q=>{const k=key(q),v=p[k]||0,done=v>=q.goal,claimed=!!c[k];return`<article class="quest-card ${done?'done':''}"><h3>${q.title}</h3><p>${q.category} · listens for <code>${q.eventType}</code></p><progress max="${q.goal}" value="${Math.min(v,q.goal)}"></progress><strong>${Math.min(v,q.goal)} / ${q.goal} · ${q.pointReward||q.reward} pts · ${q.mixerTokenReward||q.reward} Mixer Tokens${q.soundCategory?' · unlocks '+q.soundCategory:''}</strong><button data-claim="${q.id}" ${!done||claimed?'disabled':''}>${claimed?'Claimed':'Claim reward'}</button></article>`}).join('');
