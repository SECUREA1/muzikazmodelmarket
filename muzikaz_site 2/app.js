...
function hashCredential(value){
  return btoa(unescape(encodeURIComponent(value.trim().toLowerCase()))).replace(/=+$/,'');
}
...
function analyzeSnapshot(canvas){
  ...
  for(let i = 0; i < data.length; i += 4){
    ...
    if(r > 55 && g > 35 && b > 25 && r > b && Math.abs(r - g) > 8) skinLike++;
  }
  const average = total / sampleCount;
  const contrast = Math.sqrt(Math.max(0, totalSq / sampleCount - average * average));
  return {
    average: Math.round(average),
    contrast: Math.round(contrast),
    skinRatio: Number((skinLike / sampleCount).toFixed(3))
  };
}

function closeFaceStream(){
  faceStream?.getTracks().forEach(track=>track.stop());
  faceStream = null;
}

function approveAccess(snapshotUrl){
  approvalBadge.textContent = 'Approved — face validated';
  approvalBadge.classList.add('approved');
  approvalStep.classList.add('done');
  localStorage.setItem('muzikazSecureSession', JSON.stringify({
    user: loginName.value.trim(),
    approvedAt: new Date().toISOString(),
    snapshotUrl
  }));
  closeFaceStream();
  setAuthStatus('Login valid and face scan approved. Opening MUZIKAZ storefront…', 'success');
  window.setTimeout(()=>{
    authGate.classList.add('approved');
...
async function startFaceCamera(){
  if(!navigator.mediaDevices?.getUserMedia){
    setAuthStatus('Camera access is unavailable in this browser. Use HTTPS or localhost for webcam scanning.', 'error');
    return;
  }
  faceStream = await navigator.mediaDevices.getUserMedia({video:{facingMode:'user', width:{ideal:720}, height:{ideal:540}}, audio:false});
  faceVideo.srcObject = faceStream;
  await faceVideo.play();
  captureFace.disabled = false;
  setAuthStatus('Camera opened. Center your face in the scan oval and capture the snapshot.', 'success');
}
...
async function captureAndValidateFace(){
  ...
  ctx.restore();
  const snapshotUrl = faceCanvas.toDataURL('image/jpeg', .86);
  snapshotPreview.src = snapshotUrl;
  snapshotPreview.hidden = false;
  faceVideo.hidden = true;
  scanStep.classList.add('done');
  approvalBadge.textContent = 'Validating scan…';
  const signature = analyzeSnapshot(faceCanvas);
  const credentialKey = `muzikazFaceSignature:${hashCredential(loginName.value)}`;
  const stored = JSON.parse(localStorage.getItem(credentialKey) || 'null');
  const hasFaceSignal = signature.contrast >= 18 && signature.skinRatio >= .02 && signature.average >= 35;
  const matchesStored = !stored || Math.abs(stored.average - signature.average) <= 36 && Math.abs(stored.contrast - signature.contrast) <= 28 && Math.abs(stored.skinRatio - signature.skinRatio) <= .16;
  if(!hasFaceSignal){
    approvalBadge.textContent = 'Face scan not approved';
    setAuthStatus('Snapshot validation failed. Improve lighting, face the camera, and capture again.', 'error');
  ...
  if(!matchesStored){
    approvalBadge.textContent = 'Face does not match';
    setAuthStatus('This face scan does not match the enrolled snapshot for this login.', 'error');
    return;
  }
  localStorage.setItem(credentialKey, JSON.stringify(signature));
  approveAccess(snapshotUrl);
}

function initSecureGate(){
  if(!authGate || !siteShell) return;
  authForm.addEventListener('submit', event=>{
    event.preventDefault();
    loginValidated = loginName.value.trim().length >= 3 && loginCode.value.trim().toUpperCase() === AUTH_CODE;
    if(!loginValidated){
      setAuthStatus('Login denied. Check the username and access code before scanning.', 'error');
      return;
    }
    loginStep.classList.add('done');
    startCamera.disabled = false;
    setAuthStatus('Login valid. Open the camera to continue face validation.', 'success');
  });
  startCamera.addEventListener('click', ()=>startFaceCamera().catch(()=>setAuthStatus('Camera permission was denied or unavailable.', 'error')));
  captureFace.addEventListener('click', ()=>captureAndValidateFace());
}
