(function () {
  ...
  let member = null;
  const owner = () => String(member?.walletId || '').trim().toLowerCase();
  const headers = () => ({ Accept: 'application/json', 'Content-Type': 'application/json', ...(window.MuzikazAccountSession?.csrfToken ? { 'X-CSRF-Token': window.MuzikazAccountSession.csrfToken } : {}) });
  ...
  document.addEventListener('muzikaz:member-authenticated', () => restoreMember().then(() => loadMembers(owner())).catch(showError));
}());
