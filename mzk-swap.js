(function () {
  ...
  async function refresh() {
    ...
    buy.disabled = usd < PRESALE_MINIMUM_USD;
    ...
    try {
      ...
      quoteCopy.textContent = `${quote.amount} ${quote.currency} ≈ $${usd.toFixed(2)} · ${amount.toLocaleString()} MZK`;
  ...
  form.addEventListener('submit', async (event) => {
    event.preventDefault(); const usd = Number(usdInput.value); if (usd < PRESALE_MINIMUM_USD) return; buy.disabled = true;
    try {
      ...
      status.textContent = `${entry.amount.toLocaleString()} MZK added. Transaction ${payment.transactionHash.slice(0, 12)}…`;
