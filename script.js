...
function initHouseExplorer() {
  ...
  function initializeHousePresence() { const send = () => { const now = Date.now(); if (now - lastPresenceAt < 800) return; lastPresenceAt = now; const position = clampPosition({ x: camera.x, y: 0, z: camera.z }); const tracking = { position, roomId: calculateRoomId(position), avatarUrl: window.MUZIKAZ_HOUSE_TRACKING?.avatarUrl || 'logo_symbol_crop_2x_transparent.png', message: window.MUZIKAZ_HOUSE_TRACKING?.message || '' }; window.MUZIKAZ_HOUSE_TRACKING = tracking; fetch(API_BASE + '/api/houses/' + HOUSE_ID + '/presence', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-MUZIKAZ-Session': sessionId }, body: JSON.stringify({ username: (window.localStorage.getItem('muzikazBottleMemberEmail') || 'Guest').split('@')[0], ...tracking }) }).then((response) => response.ok ? response.json() : null).then((data) => data && syncLivePlayers(data)).catch(() => {}); }; send(); window.setInterval(send, 1000); window.addEventListener('muzikaz-house-chat', (event) => { const player = livePlayerObjects.get(event.detail?.sessionId); if (player) player.message = event.detail.message; }); window.addEventListener('beforeunload', () => navigator.sendBeacon?.(API_BASE + '/api/houses/' + HOUSE_ID + '/presence/leave?sessionId=' + encodeURIComponent(sessionId))); }
