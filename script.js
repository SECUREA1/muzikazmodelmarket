...
function initHouseExplorer() {
  ...
  const enterInteriorButton = document.querySelector('#house-enter-interior');
  ...
  function startHouseGame() {
    ...
    [startGameButton, enterInteriorButton].filter(Boolean).forEach((button) => { const label = button.querySelector('span'); if (label) label.textContent = 'Loading'; else button.textContent = 'Loading toxins…'; button.disabled = true; });
    window.setTimeout(() => {
      spawnToxicBubbles();
      gameStarted = true;
      ...
      [startGameButton, enterInteriorButton].filter(Boolean).forEach((button) => { const label = button.querySelector('span'); if (label) label.textContent = 'Playing'; else button.textContent = 'Playing'; button.disabled = true; button.setAttribute('aria-label', 'RAD-TOX has started'); });
      canvas.focus?.();
    }, 500);
  }
  ...
  enterInteriorButton?.addEventListener('click', startHouseGame);
