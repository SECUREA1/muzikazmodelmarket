...
(function(){
  ...
  const auth={'X-User-Id':userId,'X-User-Role':role,'X-User-Name':localStorage.getItem('muzikazName')||email||'MUZIKAZ Creator','Accept':'application/json'}; if(email) auth['X-User-Email']=email;
  ...
  async function submitUpload(form,isModel,e){e.preventDefault(); const submit=e.submitter; const progress=document.getElementById(isModel?'model-upload-progress':'graphic-upload-progress'); progress&&(progress.value=15); const fd=new FormData(), fields={}; [...form.querySelectorAll('input,textarea,select')].forEach(el=>{ if(el.type==='file'){[...(el.files||[])].forEach(f=>fd.append('file',f));} else if(el.type==='checkbox'){fd.append(el.name,el.checked?'true':'false');fields[el.name]=el.checked?'true':'false';} else if(el.name){fd.append(el.name,el.value);fields[el.name]=el.value;} }); fd.set('status',submit?.value||'draft'); fields.status=submit?.value||'draft'; status.textContent=isModel?'Processing model':'Uploading files…'; try{lastGraphic=await api(isModel?'/api/models/upload':'/api/assets/upload',{method:'POST',body:fd,headers:{}}); progress&&(progress.value=100); status.textContent= isModel?'Server upload complete. Stored GLB is persistent and available after refresh.':'Upload complete. Thumbnail generated.'; await loadAssets();}catch(err){const files=[...form.querySelectorAll('input[type=file]')].flatMap(el=>[...(el.files||[])]); if(files.length&&!isModel){const local=await Promise.all(files.map(f=>fileToAsset(f,fields,isModel))); const list=[...local,...localAssets()]; saveLocalAssets(list); lastGraphic=local[0]; progress&&(progress.value=100); status.textContent='Server unavailable; image files were saved only as browser drafts and must be retried to persist.'; await loadAssets();}else{progress&&(progress.value=0);status.textContent=(err.message||'Upload failed')+(isModel?' No playable environment was saved; use Retry Upload when the server is available.':'');}}}
