...
(function(){
  ...
  renderTabs();
  async function api(path,opts={}){const r=await fetch(path,{...opts,headers:{...auth,...(opts.headers||{})}});let j={};try{j=await r.json();}catch{throw new Error('Asset service returned an invalid response. Start the MUZIKAZ server with npm start.');}if(!r.ok||j.success===false)throw new Error(j.message||j.error||'Request failed');return j.data ?? j;}
  ...
  document.querySelectorAll('[data-tab-jump]').forEach((button)=>button.addEventListener('click',()=>{current=button.dataset.tabJump||'My Uploads';renderTabs();loadAssets();document.getElementById('asset-status')?.scrollIntoView({behavior:'smooth',block:'center'});}));
  ...
  async function loadAssets(){try{status.textContent='Loading assets…'; const list=await api(current==='Public Assets'?'/api/assets/public':'/api/assets/mine'); const view=filtered(list); grid.replaceChildren(...(view.length?view.map(card):[emptyCard(current)])); status.textContent=`${view.length} assets loaded for ${current}.`; updateAssetMetrics(list); }catch(e){status.textContent=e.message;}}
