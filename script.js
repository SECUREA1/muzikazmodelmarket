...
function initHouseExplorer() {
  ...
  const toxicBubbles = [];
  let toxicBubbleAudio = null, lastBubbleSoundAt = 0;
  ...
  window.addEventListener('resize', resizeCanvas); resizeCanvas(); spawnToxicBubbles(); render(); tickMovement(); loadEnvironmentCatalog(); window.setTimeout(initializeSharedHouseAvatars, 0);
}
