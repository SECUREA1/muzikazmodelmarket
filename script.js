...
function initHouseExplorer() {
  ...
  const houseApi = window.MUZIKAZ_API || {
    url: (path) => path,
    fetch: (path, options) => fetch(path, options)
  };
  const houseApiUrl = houseApi.url;
  const houseApiFetch = houseApi.fetch;
  ...
  fullscreenButton?.addEventListener('click', async () => {
    const gameScreen = canvas.closest('.house-explorer-shell');
