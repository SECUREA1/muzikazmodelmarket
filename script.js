...
function initHouseExplorer() {
  ...
  function clampPosition(position) { const b = activeEnvironment.bounds || defaultEnvironment.bounds; return { x: Math.max(b.minX + .35, Math.min(b.maxX - .35, Number(position.x) || 0)), y: 0, z: Math.max(b.minZ + .35, Math.min(b.maxZ - .35, Number(position.z) || 0)) }; }
  ...
  function triggerMobileMove(direction) {
    if (direction === 'left') { camera.yaw -= .22; return; }
    if (direction === 'right') { camera.yaw += .22; return; }
    if (direction === 'jump') {
      const baseY = camera.y;
      camera.y = Math.min(2.4, camera.y + .34);
      window.setTimeout(() => { camera.y = Math.max(.8, baseY); }, 180);
      return;
    }
    move(direction, .42);
  }
  ...
  window.addEventListener('resize', resizeCanvas); resizeCanvas(); loadEnvironmentCatalog(); render(); tickMovement(); initializeSharedHouseAvatars();
}
