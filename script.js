...
function initHouseExplorer() {
  ...
  const minimizeButton = document.querySelector('#house-minimize');
  const expandButton = document.querySelector('#house-expand');
  const placePersonButton = document.querySelector('#house-place-person');
  const mapFitRange = document.querySelector('#house-map-fit');
  const viewHeightRange = document.querySelector('#house-view-height');
  const viewZoomRange = document.querySelector('#house-view-zoom');
  ...
  const beginOverlay = document.querySelector('[data-house-begin-overlay]');
  const beginButton = document.querySelector('#house-begin-button');
  const environmentSelect = document.querySelector('#house-environment-select');
  const environmentUpload = document.querySelector('#house-environment-upload');
  ...
  let dragging = false, draggedAvatar = null, lastPointer = null, handEnabled = false, handStream = null, handController = null, lastPresenceAt = 0, gameStarted = false;
  ...
  function requireGameStart(action = 'Press Enter on screen to begin the house game.') {
    if (gameStarted) return true;
    setStatus(action);
    beginButton?.focus();
    return false;
  }
  function beginHouseGame() {
    if (gameStarted) return;
    gameStarted = true;
    beginOverlay?.setAttribute('hidden', '');
    canvas.focus();
    setStatus('Game started: drag to look around, then move through the rooms.');
  }
  ...
  function syncTunerControls() { if (mapFitRange) mapFitRange.value = String(Math.round(mapFit * 100)); if (viewHeightRange) viewHeightRange.value = String(Math.round(camera.y * 100)); if (viewZoomRange) viewZoomRange.value = String(Math.round(camera.fov)); }
  function applyEnvironment(raw, source = 'environment file') { activeEnvironment = normalizeEnvironment(raw); walls = activeEnvironment.walls; demoAvatars = activeEnvironment.demoAvatars; defaultCamera = { ...activeEnvironment.camera }; Object.assign(camera, defaultCamera); syncTunerControls(); setStatus(activeEnvironment.name + ' loaded from ' + source + ' as a controlled map.'); }
  ...
  document.querySelectorAll('[data-mobile-hold]').forEach((button) => {
    ...
    const start = (event) => { event.preventDefault(); if (!requireGameStart()) return; mobileControlState.add(action); button.classList.add('is-active'); applyMobileControl(action, 1.6); };
