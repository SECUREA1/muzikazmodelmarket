...
function ownedAssetDetail(assetName) {
  ...
  if (dollKit) return { title: assetName, type: dollKit.category, image: dollKit.asset, copy: `Avatar Flex Lab asset compatible with ${dollKit.connectsTo.join(' + ')} and tied to the owner's account.` };
...
const assetCatalog = {
  ...
  dollKitAssets: [
    { id: 'avatar-loadout', name: 'Avatar Loadout Shelf', category: 'Avatars', price: '$26.00', asset: 'trait_avatars_row_1_2x.png', connectsTo: ['Trait Avatars', 'New Legends'], copy: 'Base bodies, alternate skins, expressions, profile poses, and trait-ready avatar slots for character-specific Avatar Flex Lab builds.' },
...
const marketplaceListings = [
  ...
  ...assetCatalog.dollKitAssets.map((asset) => ({ type: 'Avatar Flex Assets', category: asset.category, quality: 'curated', name: asset.name, price: asset.price, copy: `${asset.copy} Compatible with ${asset.connectsTo.join(' + ')}. Source: ${asset.asset}`, product: asset.name })),
...
document.addEventListener('click', (event) => {
  ...
  if (!jump) return;
  const targetType = jump.dataset.marketJump === 'Website Packages' ? 'Website Packages' : jump.dataset.marketJump === 'Doll Kit Assets' ? 'Avatar Flex Assets' : 'Subscriber Creator Tools';
  marketplaceState.category = 'All';
...
function initHouseExplorer() {
  ...
  const radToxGame = { enabled: true, bubblesEnabled: true, soundEnabled: true, running: true, difficulty: 'normal', score: 0, hp: 100, maxHp: 100, invulnerableUntil: 0, bubblePool: [], activeBubbleLimit: 5, beams: [], sparks: [], gasPuffs: [], targetLock: { id: null, warmTime: 0, active: false }, audioUnlocked: false, lastFrameTime: performance.now(), settings: { speed: 1, lockEnabled: true, lockStrength: .75, pinchThreshold: .09 } };
  const toxicBubbles = radToxGame.bubblePool;
  const handFireState = { pinched: false, lastShotAt: 0, reticle: null };
  let radToxAudio = null;
  ...
  function getRadToxMapKey() { return 'muzikazRadToxProgress:' + safeText(activeEnvironment.id || activeEnvironment.name || HOUSE_ID, HOUSE_ID); }
  function readRadToxPreferences() { const get = (key, fallback) => { const value = window.localStorage.getItem(key); return value === null ? fallback : value === 'true'; }; radToxGame.bubblesEnabled = get('muzikazToxicBubblesEnabled', true); radToxGame.enabled = get('muzikazRadToxGameEnabled', true); radToxGame.soundEnabled = get('muzikazRadToxSoundEnabled', true); radToxGame.difficulty = window.localStorage.getItem('muzikazRadToxDifficulty') || 'normal'; }
  function saveRadToxPreferences() { window.localStorage.setItem('muzikazToxicBubblesEnabled', String(radToxGame.bubblesEnabled)); window.localStorage.setItem('muzikazRadToxGameEnabled', String(radToxGame.enabled)); window.localStorage.setItem('muzikazRadToxSoundEnabled', String(radToxGame.soundEnabled)); window.localStorage.setItem('muzikazRadToxDifficulty', radToxGame.difficulty); }
  function readRadToxMapState() { try { return JSON.parse(window.localStorage.getItem(getRadToxMapKey()) || 'null'); } catch (error) { return null; } }
  function saveCurrentRadToxMapState() { const bubbles = {}; toxicBubbles.forEach((bubble) => { bubbles[bubble.id] = { health: bubble.health, destroyed: bubble.destroyed, x: bubble.x, y: bubble.y, z: bubble.z, active: bubble.active }; }); window.localStorage.setItem(getRadToxMapKey(), JSON.stringify({ poolSize: toxicBubbles.length, bubbles, score: radToxGame.score, hp: radToxGame.hp, completed: radToxGame.completed || false, completedAt: radToxGame.completedAt || null })); }
  function randomInRange(min, max) { return min + Math.random() * (max - min); }
  function createToxicBubblePool(saved) { const b = activeEnvironment.bounds || defaultEnvironment.bounds; const configured = Number(activeEnvironment.toxicBubbleCount); const count = Math.max(5, Math.min(10, saved?.poolSize || configured || (5 + Math.floor(Math.random() * 6)))); toxicBubbles.length = 0; for (let i = 0; i < count; i += 1) { const id = 'toxic-bubble-' + i, old = saved?.bubbles?.[id], angle = randomInRange(0, Math.PI * 2); toxicBubbles.push({ id, health: Math.max(0, Math.min(100, Number(old?.health ?? 100))), maxHealth: 100, destroyed: Boolean(old?.destroyed) || Number(old?.health) <= 0, active: false, x: Number(old?.x) || randomInRange(b.minX + .8, b.maxX - .8), y: Number(old?.y) || randomInRange(.7, 1.85), z: Number(old?.z) || randomInRange(b.minZ + .8, b.maxZ - .8), radius: randomInRange(.24, .42), velocityX: Math.cos(angle) * randomInRange(.25, .55), velocityZ: Math.sin(angle) * randomInRange(.25, .55), phase: randomInRange(0, Math.PI * 2), visualSeed: Math.random(), flash: 0 }); } }
  function getActiveToxicBubbles() { return toxicBubbles.filter((bubble) => bubble.active && !bubble.destroyed); }
  function restoreActiveBubbleSlots() { let slots = 0; toxicBubbles.forEach((bubble) => { bubble.active = false; }); toxicBubbles.forEach((bubble) => { if (!bubble.destroyed && slots < radToxGame.activeBubbleLimit) { bubble.active = true; slots += 1; } }); }
  function activateNextBubble() { const next = toxicBubbles.find((bubble) => !bubble.destroyed && !bubble.active); if (next && getActiveToxicBubbles().length < radToxGame.activeBubbleLimit) next.active = true; }
  function spawnToxicBubbles() { loadRadToxMapState(); }
  function loadRadToxMapState() { const saved = readRadToxMapState(); createToxicBubblePool(saved); radToxGame.score = Number(saved?.score) || 0; radToxGame.hp = Math.max(0, Math.min(100, Number(saved?.hp ?? 100))); radToxGame.completed = Boolean(saved?.completed); radToxGame.completedAt = saved?.completedAt || null; restoreActiveBubbleSlots(); saveCurrentRadToxMapState(); }
  function resetCurrentRadToxMap() { if (!window.confirm('Reset RAD-TOX progress for this map only?')) return; window.localStorage.removeItem(getRadToxMapKey()); loadRadToxMapState(); radToxGame.hp = 100; updateRadToxHUD(); setStatus('RAD-TOX map progress reset. Five active targets restored.'); }
  function applyEnvironment(raw, source = 'environment file') { saveCurrentRadToxMapState(); activeEnvironment = normalizeEnvironment(raw); walls = activeEnvironment.walls; demoAvatars = activeEnvironment.demoAvatars; defaultCamera = { ...activeEnvironment.camera }; Object.assign(camera, defaultCamera); radToxGame.targetLock = { id: null, warmTime: 0, active: false }; loadRadToxMapState(); updateRadToxHUD(); setStatus(activeEnvironment.name + ' loaded from ' + source + '. RAD-TOX map progress restored.'); }
  ...
  async function startMediaPipeHands() { await Promise.all([loadScriptOnce('https://cdn.jsdelivr.net/npm/@mediapipe/hands/hands.js'), loadScriptOnce('https://cdn.jsdelivr.net/npm/@mediapipe/camera_utils/camera_utils.js')]); if (!window.Hands || !window.Camera || !preview) return false; const hands = new window.Hands({ locateFile: (file) => 'https://cdn.jsdelivr.net/npm/@mediapipe/hands/' + file }); hands.setOptions({ maxNumHands: 1, modelComplexity: 0, minDetectionConfidence: .65, minTrackingConfidence: .55 }); hands.onResults((results) => { const points = results.multiHandLandmarks?.[0], tip = points?.[8], thumb = points?.[4]; if (!tip) return; camera.yaw += (tip.x - .5) * .035; camera.pitch = Math.max(-.8, Math.min(.55, camera.pitch + (tip.y - .5) * .025)); handFireState.reticle = { x: tip.x * canvas.clientWidth, y: tip.y * canvas.clientHeight }; if (tip.y < .34) move('forward', .08); if (tip.y > .72) move('back', .08); const pinched = thumb && Math.hypot(tip.x - thumb.x, tip.y - thumb.y) < radToxGame.settings.pinchThreshold; if (pinched && !handFireState.pinched && performance.now() - handFireState.lastShotAt > 350) { const target = getActiveToxicBubbles().map((bubble) => ({ bubble, p: project(bubble) })).filter((item) => item.p).sort((a,b) => Math.hypot(a.p.x - handFireState.reticle.x, a.p.y - handFireState.reticle.y) - Math.hypot(b.p.x - handFireState.reticle.x, b.p.y - handFireState.reticle.y))[0]; if (target) { damageToxicBubble(target.bubble.id); handFireState.lastShotAt = performance.now(); } } handFireState.pinched = Boolean(pinched); }); handController = new window.Camera(preview, { onFrame: async () => hands.send({ image: preview }), width: 320, height: 180 }); handController.start(); return true; }
  ...
  class HouseRadToxAudio { constructor() { this.context = null; this.unlocked = false; this.enabled = true; this.masterGain = null; this.ambienceGain = null; this.sfxGain = null; this.lastSoundAt = 0; } async unlock() { if (this.unlocked) return; const Ctx = window.AudioContext || window.webkitAudioContext; if (!Ctx) return; try { this.context = new Ctx(); this.masterGain = this.context.createGain(); this.ambienceGain = this.context.createGain(); this.sfxGain = this.context.createGain(); this.masterGain.gain.value = .7; this.ambienceGain.gain.value = .35; this.sfxGain.gain.value = .8; this.ambienceGain.connect(this.masterGain); this.sfxGain.connect(this.masterGain); this.masterGain.connect(this.context.destination); await this.context.resume(); this.unlocked = true; } catch (error) {} } setEnabled(enabled) { this.enabled = enabled; } play(type) { if (!this.unlocked || !this.enabled || !this.context) return; const now = this.context.currentTime; if (type === 'ambient' && performance.now() - this.lastSoundAt < 2400) return; this.lastSoundAt = performance.now(); const osc = this.context.createOscillator(), gain = this.context.createGain(); osc.type = type === 'destroy' ? 'sawtooth' : 'sine'; osc.frequency.setValueAtTime(type === 'hurt' ? 110 : type === 'destroy' ? 170 : 320, now); osc.frequency.exponentialRampToValueAtTime(type === 'destroy' ? 40 : 70, now + .18); gain.gain.setValueAtTime(type === 'ambient' ? .018 : .055, now); gain.gain.exponentialRampToValueAtTime(.0001, now + .25); osc.connect(gain).connect(type === 'ambient' ? this.ambienceGain : this.sfxGain); osc.start(now); osc.stop(now + .27); } }
  function getRadToxDifficultySettings() { return ({ easy: { speed: .75, contactDamage: 0, multiplier: 1 }, normal: { speed: 1, contactDamage: 3, multiplier: 1 }, hard: { speed: 1.25, contactDamage: 6, multiplier: 2 }, nightmare: { speed: 1.5, contactDamage: 10, multiplier: 3 } })[radToxGame.difficulty] || { speed: 1, contactDamage: 3, multiplier: 1 }; }
  function updateRadToxHUD() { const set = (id, value) => { const el = document.querySelector(id); if (el) el.textContent = value; }; const active = getActiveToxicBubbles().length, remaining = toxicBubbles.filter((b) => !b.destroyed).length; set('#house-game-hp-text', radToxGame.hp); const fill = document.querySelector('#house-game-hp-fill'); if (fill) fill.style.width = (radToxGame.hp / radToxGame.maxHp * 100) + '%'; set('#house-game-score', radToxGame.score); set('#house-game-active-count', active); set('#house-game-remaining-count', remaining); set('#house-game-sound', 'Sound ' + (radToxGame.soundEnabled ? 'On' : 'Off')); set('#house-game-difficulty', radToxGame.difficulty); set('#house-game-state', !radToxGame.bubblesEnabled ? 'BUBBLES OFF' : !radToxGame.enabled ? 'AMBIENT' : !radToxGame.running ? 'FIELD DOWN' : radToxGame.completed ? 'CLEARED' : 'RUNNING'); }
  function updateToxicBubbles(deltaSeconds) { if (!radToxGame.bubblesEnabled || document.hidden) return; const b = activeEnvironment.bounds || defaultEnvironment.bounds, speed = getRadToxDifficultySettings().speed * radToxGame.settings.speed; getActiveToxicBubbles().forEach((bubble) => { bubble.phase += deltaSeconds * 2; bubble.x += bubble.velocityX * deltaSeconds * speed; bubble.z += bubble.velocityZ * deltaSeconds * speed; bubble.y = Math.max(.55, Math.min(2.1, 1.25 + Math.sin(bubble.phase) * .55)); if (bubble.x < b.minX + .6 || bubble.x > b.maxX - .6) bubble.velocityX *= -1; if (bubble.z < b.minZ + .6 || bubble.z > b.maxZ - .6) bubble.velocityZ *= -1; bubble.x = Math.max(b.minX + .6, Math.min(b.maxX - .6, bubble.x)); bubble.z = Math.max(b.minZ + .6, Math.min(b.maxZ - .6, bubble.z)); bubble.flash = Math.max(0, bubble.flash - deltaSeconds * 3); }); if (radToxAudio && toxicBubbles.some((bubble) => bubble.active && !bubble.destroyed)) radToxAudio.play('ambient'); }
  function updatePlayerToxicDamage(now) { if (!radToxGame.enabled || !radToxGame.bubblesEnabled || !radToxGame.running || now < radToxGame.invulnerableUntil) return; const damage = getRadToxDifficultySettings().contactDamage; const touching = getActiveToxicBubbles().some((bubble) => Math.hypot(camera.x - bubble.x, camera.z - bubble.z) < .8); if (touching && damage) { radToxGame.hp = Math.max(0, radToxGame.hp - damage); radToxGame.invulnerableUntil = now + 700; radToxAudio?.play('hurt'); if (!radToxGame.hp) { radToxGame.running = false; setStatus('FIELD DOWN. Restart Run restores HP without erasing map progress.'); } saveCurrentRadToxMapState(); updateRadToxHUD(); } }
  function spawnEffects(bubble, destroyed) { const p = project(bubble); if (!p) return; radToxGame.beams.push({ x: canvas.clientWidth / 2, y: canvas.clientHeight / 2, tx: p.x, ty: p.y, life: .16 }); for (let i = 0; i < (destroyed ? 15 : 6); i += 1) radToxGame.sparks.push({ x: p.x, y: p.y, vx: randomInRange(-60, 60), vy: randomInRange(-60, 60), life: destroyed ? .7 : .35 }); if (destroyed) radToxGame.gasPuffs.push({ x: p.x, y: p.y, life: 1, size: 12 }); radToxGame.sparks.splice(0, Math.max(0, radToxGame.sparks.length - 60)); }
  function updateRadToxEffects(deltaSeconds) { ['beams', 'sparks', 'gasPuffs'].forEach((key) => { radToxGame[key] = radToxGame[key].filter((item) => { item.life -= deltaSeconds; if (key === 'sparks') { item.x += item.vx * deltaSeconds; item.y += item.vy * deltaSeconds; } if (key === 'gasPuffs') item.size += 28 * deltaSeconds; return item.life > 0; }); }); }
  function damageToxicBubble(id, amount = 25) { if (!radToxGame.enabled || !radToxGame.bubblesEnabled || avatarPlacementState.active || !radToxGame.running) return; const bubble = toxicBubbles.find((item) => item.id === id); if (!bubble || bubble.destroyed || !bubble.active) return; bubble.health = Math.max(0, bubble.health - amount); bubble.flash = 1; const destroyed = bubble.health === 0; spawnEffects(bubble, destroyed); if (destroyed) { bubble.destroyed = true; bubble.active = false; radToxGame.score += getRadToxDifficultySettings().multiplier; activateNextBubble(); radToxAudio?.play('destroy'); if (!toxicBubbles.some((item) => !item.destroyed)) { radToxGame.completed = true; radToxGame.completedAt = new Date().toISOString(); radToxGame.score += 5 * getRadToxDifficultySettings().multiplier; setStatus('RAD-TOX map cleared. Completion bonus secured.'); } } else radToxAudio?.play('hit'); saveCurrentRadToxMapState(); updateRadToxHUD(); }
  function drawToxicBubble(bubble) { const p = project(bubble); if (!p) return null; const size = Math.max(18, (bubble.radius * 520) / p.d), hue = bubble.visualSeed > .5 ? '255,210,40' : '90,255,0'; ctx.save(); ctx.translate(p.x, p.y); ctx.globalAlpha = .84; ctx.shadowBlur = 24; ctx.shadowColor = 'rgba(' + hue + ',.75)'; const ooze = ctx.createRadialGradient(-size * .28, -size * .32, size * .08, 0, 0, size); ooze.addColorStop(0, 'rgba(235,255,150,.95)'); ooze.addColorStop(.45, 'rgba(' + hue + ',.68)'); ooze.addColorStop(1, 'rgba(15,60,16,.38)'); ctx.fillStyle = ooze; ctx.beginPath(); ctx.arc(0, 0, size * (1 + Math.sin(bubble.phase) * .04), 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = 'rgba(36,8,50,.5)'; for (let i = 0; i < 3; i += 1) { ctx.beginPath(); ctx.arc(Math.sin(bubble.phase + i) * size * .44, Math.cos(bubble.phase * .7 + i) * size * .35, size * .08, 0, Math.PI * 2); ctx.fill(); } if (bubble.flash) { ctx.globalAlpha = bubble.flash; ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(0, 0, size, 0, Math.PI * 2); ctx.fill(); } if (radToxGame.enabled) { const w = size * 1.7, h = Math.max(4, size * .12); ctx.globalAlpha = 1; ctx.shadowBlur = 0; ctx.fillStyle = 'rgba(0,0,0,.75)'; ctx.fillRect(-w / 2, -size - 13, w, h); ctx.fillStyle = bubble.health > 55 ? '#9cff00' : bubble.health > 25 ? '#ffda3a' : '#ff2e2e'; ctx.fillRect(-w / 2, -size - 13, w * bubble.health / 100, h); } ctx.restore(); return { id: bubble.id, x: p.x, y: p.y, width: size * 2, height: size * 2, depth: p.d, kind: 'toxic-bubble' }; }
  function drawRadToxEffects() { ctx.save(); radToxGame.beams.forEach((beam) => { ctx.globalAlpha = beam.life / .16; ctx.strokeStyle = '#baff32'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(beam.x, beam.y); ctx.lineTo(beam.tx, beam.ty); ctx.stroke(); }); radToxGame.sparks.forEach((spark) => { ctx.globalAlpha = spark.life * 2; ctx.fillStyle = '#dfff59'; ctx.fillRect(spark.x - 2, spark.y - 2, 4, 4); }); radToxGame.gasPuffs.forEach((puff) => { ctx.globalAlpha = puff.life * .35; ctx.strokeStyle = '#9cff00'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(puff.x, puff.y, puff.size * (1 - puff.life + .3), 0, Math.PI * 2); ctx.stroke(); }); ctx.restore(); }
  function render() { const now = performance.now(), deltaSeconds = Math.min(50, now - radToxGame.lastFrameTime) / 1000; radToxGame.lastFrameTime = now; updateToxicBubbles(deltaSeconds); updatePlayerToxicDamage(now); updateRadToxEffects(deltaSeconds); const rect = canvas.getBoundingClientRect(); ctx.clearRect(0, 0, rect.width, rect.height); const gradient = ctx.createLinearGradient(0, 0, 0, rect.height); gradient.addColorStop(0, '#06110c'); gradient.addColorStop(1, '#010201'); ctx.fillStyle = gradient; ctx.fillRect(0, 0, rect.width, rect.height); { const b = activeEnvironment.bounds; drawPolygon([{x:b.minX,y:0,z:b.minZ},{x:b.maxX,y:0,z:b.minZ},{x:b.maxX,y:0,z:b.maxZ},{x:b.minX,y:0,z:b.maxZ}], activeEnvironment.floorColor); drawPolygon([{x:b.minX,y:3,z:b.minZ},{x:b.minX,y:3,z:b.maxZ},{x:b.maxX,y:3,z:b.maxZ},{x:b.maxX,y:3,z:b.minZ}], activeEnvironment.ceilingColor); walls.forEach(([a, c]) => drawPolygon([{x:a[0],y:0,z:a[1]},{x:c[0],y:0,z:c[1]},{x:c[0],y:2.7,z:c[1]},{x:a[0],y:2.7,z:a[1]}], activeEnvironment.wallColor, activeEnvironment.accentColor)); } const hitboxes = []; if (radToxGame.bubblesEnabled) getActiveToxicBubbles().map((bubble) => ({ bubble, p: project(bubble) })).filter((item) => item.p).sort((a,b) => b.p.d - a.p.d).forEach(({bubble}) => { const box = drawToxicBubble(bubble); if (box) hitboxes.push(box); }); [...sharedAvatarObjects.values()].forEach((object) => { const box = drawAvatarObject(object); if (box) hitboxes.push({ ...box, id: object.record.id, kind: 'shared-avatar' }); }); if (avatarPlacementState.active && avatarPlacementState.previewObject) drawAvatarObject(avatarPlacementState.previewObject, true); drawRadToxEffects(); canvas._avatarHitboxes = hitboxes; requestAnimationFrame(render); }
  ...
  const bubbleToggle = document.querySelector('#toxic-bubble-toggle'), runToggle = document.querySelector('#radtox-run-toggle'), soundToggle = document.querySelector('#radtox-sound-toggle'), difficultySelect = document.querySelector('#radtox-difficulty');
  function syncRadToxControls() { [[bubbleToggle, radToxGame.bubblesEnabled, 'Toxic Bubbles'], [runToggle, radToxGame.enabled, 'RAD-TOX Game'], [soundToggle, radToxGame.soundEnabled, 'Sound']].forEach(([button, value, name]) => { if (button) { button.textContent = name + ': ' + (value ? 'On' : 'Off'); button.setAttribute('aria-pressed', String(value)); } }); if (difficultySelect) difficultySelect.value = radToxGame.difficulty; updateRadToxHUD(); }
  function unlockRadToxAudio() { if (!radToxGame.soundEnabled) return; radToxAudio ||= new HouseRadToxAudio(); radToxAudio.enabled = true; radToxAudio.unlock(); }
  canvas.addEventListener('pointerdown', unlockRadToxAudio, { once: true });
  bubbleToggle?.addEventListener('click', () => { radToxGame.bubblesEnabled = !radToxGame.bubblesEnabled; saveRadToxPreferences(); syncRadToxControls(); setStatus(radToxGame.bubblesEnabled ? 'Toxic bubbles enabled. Five active RAD-TOX targets restored.' : 'Toxic bubbles disabled. Map progress remains saved.'); });
  runToggle?.addEventListener('click', () => { radToxGame.enabled = !radToxGame.enabled; if (radToxGame.enabled && radToxGame.hp > 0) radToxGame.running = true; saveRadToxPreferences(); syncRadToxControls(); });
  soundToggle?.addEventListener('click', () => { radToxGame.soundEnabled = !radToxGame.soundEnabled; if (radToxAudio) radToxAudio.setEnabled(radToxGame.soundEnabled); saveRadToxPreferences(); syncRadToxControls(); });
  document.querySelector('#radtox-restart')?.addEventListener('click', () => { radToxGame.hp = 100; radToxGame.running = true; radToxGame.beams = []; radToxGame.sparks = []; radToxGame.gasPuffs = []; saveCurrentRadToxMapState(); syncRadToxControls(); setStatus('RAD-TOX run restarted. Map progress preserved.'); });
  document.querySelector('#radtox-reset-map')?.addEventListener('click', resetCurrentRadToxMap);
  difficultySelect?.addEventListener('change', () => { radToxGame.difficulty = difficultySelect.value; saveRadToxPreferences(); syncRadToxControls(); });
  document.querySelector('#radtox-speed')?.addEventListener('input', (event) => { radToxGame.settings.speed = Number(event.target.value); });
  document.querySelector('#radtox-lock-enabled')?.addEventListener('change', (event) => { radToxGame.settings.lockEnabled = event.target.checked; });
  document.querySelector('#radtox-lock-strength')?.addEventListener('input', (event) => { radToxGame.settings.lockStrength = Number(event.target.value); });
  document.querySelector('#radtox-pinch-threshold')?.addEventListener('input', (event) => { radToxGame.settings.pinchThreshold = Number(event.target.value); });
  document.querySelector('#radtox-master-volume')?.addEventListener('input', (event) => { if (radToxAudio?.masterGain) radToxAudio.masterGain.gain.value = Number(event.target.value); });
  readRadToxPreferences(); window.addEventListener('resize', resizeCanvas); resizeCanvas(); loadRadToxMapState(); syncRadToxControls(); render(); tickMovement(); loadEnvironmentCatalog(); window.setTimeout(initializeSharedHouseAvatars, 0);
}
