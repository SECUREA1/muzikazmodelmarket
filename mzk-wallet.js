(function () {
  ...
  const SWAP_BACK_RATE = 0.6;
  const MINIMUM_SWAP_BACK_MZK = 100;
  ...
  function completeSwapBack(amount, payout = {}) { amount = Math.floor(Number(amount)); if (!Number.isFinite(amount) || amount < MINIMUM_SWAP_BACK_MZK) throw new Error(`Swap back at least ${MINIMUM_SWAP_BACK_MZK.toLocaleString()} MZK.`); if (!payout.transactionHash) throw new Error('The crypto payout was not confirmed. Your MZK was not changed.'); const usd = amount / MZK_PER_USD * SWAP_BACK_RATE; return spend(amount, `${amount.toLocaleString()} MZK swapped back for ${payout.currency || 'crypto'} at 60% value`, `mzk:swap-back:${payout.transactionHash}`, { kind: 'swap-back', usd, payoutCurrency: payout.currency || '', payoutAmount: payout.amount, transactionHash: payout.transactionHash }); }
  ...
  window.MZKWallet = { symbol: 'MZK', MZK_PER_USD, SWAP_BACK_RATE, MINIMUM_SWAP_BACK_MZK, MINIMUM_PURCHASE_USD, GAME_ENTRY_MZK, walletId, balance, history, record, spend, transfer, creditPurchase, completeSwapBack, starterLoadout, claimStarterLoadout, ensureWallet, mount, profile: activeProfile, connectedAddress, connectBrowserWallet, disconnectBrowserWallet, connectIdentity, setUsername, exportWallet, importWallet, downloadWallet };
})();
