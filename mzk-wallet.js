(function () {
  ...
  function provisionStandardLoadout(account = {}) {
    ...
    const all = loadouts();
    if (!all[owner]) {
      all[owner] = { id: `account-loadout-${account.accountId || owner}`, owner, avatar: (account.gameAssets || []).find((asset) => /avatar/i.test(String(asset))) || 'Starter Avatar', land: (account.landAssets || [])[0] || 'Unrevealed MUZIKAZ Land', assets: [...new Set([...(account.gameAssets || []), ...(account.landAssets || []), ...(account.bottleClaims || [])])], entryPaidMzk: 0, accessType: account.loadoutStatus || 'included', claimedAt: account.updatedAt || new Date().toISOString() };
