...
createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/houses/ioncore-house/presence' && req.method === 'POST') { const id = session(req); const body = await bodyJson(req).catch(() => ({})); activePresence(); if (!presence.has(id) && presence.size >= maxHouseUsers) return sendJson(res, 409, { error: 'This Vibe Crib server is full.', ...presencePayload() }); const now = new Date().toISOString(); const previous = presence.get(id) || {}; const position = { x: clampNumber(body.position?.x, -4.65, 4.65, previous.position?.x || 0), y: clampNumber(body.position?.y, 0, 2.4, previous.position?.y || 0), z: clampNumber(body.position?.z, .35, 8.65, previous.position?.z || 2.5) }; presence.set(id, { sessionId: id, username: cleanText(body.username, previous.username || 'Subscriber'), joinedAt: previous.joinedAt || now, lastActiveAt: now, roomId: roomId(position), color: cleanText(body.color, previous.color || '#9cff00'), avatarUrl: cleanText(body.avatarUrl, previous.avatarUrl || 'logo_symbol_crop_2x_transparent.png'), message: cleanText(body.message, previous.message || ''), position }); const data = presencePayload(); broadcast('house-presence-updated', data); return sendJson(res, 200, data); }
