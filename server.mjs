...
async function multiplayerContext(req, res) {
  ...
  const publicDemo = req.headers['x-muzikaz-demo'] === 'public' || requestUrl.searchParams.get('demo') === 'public';
  if (publicDemo) {
    const demoSessionId = session(req);
    return { active: null, account: { accountId: `demo:${demoSessionId}`, username: user(req).name }, ownsGenieBottle: false, publicDemo: true };
  }
  const active = await accountSession(req, res); if (!active) return null;
  await loadoutCodeStore.repairEntitledAccount(active.accountId);
  const account = await loadoutCodeStore.getAccount(active.accountId);
  if (!account) { authorizationError(res, 401, 'ACCOUNT_NOT_FOUND', 'The session account no longer exists.', 'account'); return null; }
  return { active, account, ownsGenieBottle: false, publicDemo: false };
}
