...
function optionalAccountSession(req) { const token = accountSessionToken(req); const current = accountSessions.get(token); if (!current || current.expiresAt <= Date.now()) { if (token) accountSessions.delete(token); return null; } return current; }
function accountSession(req, res, csrf = false) { const token = accountSessionToken(req); const current = accountSessions.get(token); if (!current || current.expiresAt <= Date.now()) { accountSessions.delete(token); sendJson(res, 401, { success: false, code: 'SESSION_REQUIRED', message: 'Your account session is missing or expired.' }); return null; } if (csrf) { const supplied = String(req.headers['x-csrf-token'] || ''); const a = Buffer.from(supplied); const b = Buffer.from(current.csrfToken); if (a.length !== b.length || !timingSafeEqual(a, b)) { sendJson(res, 403, { success: false, code: 'CSRF_INVALID', message: 'Invalid CSRF token.' }); return null; } } return current; }
