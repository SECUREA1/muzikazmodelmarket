...
const spacesFile = join(dataDir, 'subscriber-spaces.json');
...
async function ensureStorage() { await mkdir(dataDir, { recursive: true }); await mkdir(uploadDir, { recursive: true }); await mkdir(assetUploadDir, { recursive: true }); try { await stat(dataFile); } catch { await writeFile(dataFile, '[]'); } try { await stat(assetsFile); } catch { await writeFile(assetsFile, '[]'); } try { await stat(modelsFile); } catch { await writeFile(modelsFile, '[]'); } try { await stat(spacesFile); } catch { await writeFile(spacesFile, '[]'); } }
...
async function readSpaces() { await ensureStorage(); return JSON.parse(await readFile(spacesFile, 'utf8')); }
async function writeSpaces(records) { await ensureStorage(); await writeFile(spacesFile, JSON.stringify(records, null, 2)); }
...
function cleanLongText(value, fallback = '') { return String(value || fallback).replace(/[<>]/g, '').slice(0, 1000); }
function isSafeModelUrl(value) { try { const url = new URL(String(value || ''), 'http://localhost'); return /\.(glb|gltf)(\?|$)/i.test(url.pathname) && !/^(javascript|data:text\/html)/i.test(String(value || '')) && (url.protocol === 'https:' || url.protocol === 'http:' || url.pathname.startsWith('/public/') || url.pathname.startsWith('/uploads/') || url.pathname.startsWith('/models/') || !url.host); } catch { return false; } }
function vector(input = {}, fallback = {}) { return { x: clampNumber(input.x, -10000, 10000, fallback.x || 0), y: clampNumber(input.y, -10000, 10000, fallback.y || 0), z: clampNumber(input.z, -10000, 10000, fallback.z || 0) }; }
function sanitizeSceneObject(item = {}) { const modelUrl = String(item.modelUrl || item.model_url || '').trim(); if (!isSafeModelUrl(modelUrl)) throw new Error('Scene object model URL must be a GLB or GLTF URL'); const scale = vector(item.scale, { x: 1, y: 1, z: 1 }); scale.x = clampNumber(scale.x, 0.01, 50, 1); scale.y = clampNumber(scale.y, 0.01, 50, 1); scale.z = clampNumber(scale.z, 0.01, 50, 1); return { instanceId: cleanText(item.instanceId || item.id, 'scene-object-' + randomUUID()), assetId: cleanText(item.assetId, ''), name: cleanText(item.name, 'Scene object'), assetType: cleanText(item.assetType, 'object'), category: cleanText(item.category, ''), modelUrl, thumbnailUrl: cleanText(item.thumbnailUrl, ''), position: vector(item.position), rotation: vector(item.rotation), scale, visible: item.visible !== false, locked: item.locked === true }; }
function spaceRecord(input = {}, actor, existing = null) { const now = new Date().toISOString(); const environment = input.environment || existing?.environment || {}; const environmentUrl = String(environment.modelUrl || environment.model || '').trim(); if (!isSafeModelUrl(environmentUrl)) throw new Error('A valid GLB/GLTF environment is required'); const status = ['published', 'draft'].includes(input.status) ? input.status : (existing?.status || 'draft'); return { schemaVersion: 1, spaceId: cleanText(input.spaceId || existing?.spaceId, randomUUID()), ownerId: existing?.ownerId || actor.id, name: cleanText(input.name || existing?.name, 'My 3D Space'), description: cleanLongText(input.description || existing?.description, ''), status, visibility: cleanText(input.visibility || existing?.visibility, status === 'published' ? 'public' : 'private'), thumbnailUrl: cleanText(input.thumbnailUrl || environment.thumbnailUrl || existing?.thumbnailUrl, ''), environment: { assetId: cleanText(environment.assetId || environment.id || ''), name: cleanText(environment.name || ''), assetType: 'environment', category: cleanText(environment.category || ''), modelUrl: environmentUrl, thumbnailUrl: cleanText(environment.thumbnailUrl || environment.thumbnail || '') }, objects: Array.isArray(input.objects) ? input.objects.map(sanitizeSceneObject).slice(0, 500) : (existing?.objects || []), camera: { position: vector(input.camera?.position || existing?.camera?.position, { x: 0, y: 2, z: 5 }), target: vector(input.camera?.target || existing?.camera?.target, { x: 0, y: 1, z: 0 }) }, settings: { gridEnabled: Boolean(input.settings?.gridEnabled ?? existing?.settings?.gridEnabled), collisionEnabled: input.settings?.collisionEnabled !== false, lightingPreset: cleanText(input.settings?.lightingPreset || existing?.settings?.lightingPreset, 'environment'), spawnPoint: vector(input.settings?.spawnPoint || existing?.settings?.spawnPoint, { x: 0, y: 1, z: 3 }) }, createdAt: existing?.createdAt || now, updatedAt: now, publishedAt: status === 'published' ? (input.publishedAt || now) : (status === 'draft' ? existing?.publishedAt || null : null) }; }
function canEditSpace(space, actor) { return space.ownerId === actor.id || actor.role === 'admin'; }
...
createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/spaces' && req.method === 'GET') { const actor = user(req); const records = await readSpaces(); return sendJson(res, 200, assetResponse(records.filter((s) => canEditSpace(s, actor)).sort((a,b)=>String(b.updatedAt||'').localeCompare(String(a.updatedAt||''))))); }
    if (url.pathname === '/api/spaces/published' && req.method === 'GET') { const records = await readSpaces(); return sendJson(res, 200, assetResponse(records.filter((s) => s.status === 'published' && s.visibility !== 'private'))); }
    if (url.pathname === '/api/spaces' && req.method === 'POST') { const actor = user(req); const spaces = await readSpaces(); const input = await bodyJson(req); const existing = input.spaceId ? spaces.find((s) => s.spaceId === input.spaceId) : null; if (existing && !canEditSpace(existing, actor)) return sendJson(res, 403, { success: false, message: 'Forbidden' }); const record = spaceRecord(input, actor, existing); const index = spaces.findIndex((s) => s.spaceId === record.spaceId); if (index >= 0) spaces[index] = record; else spaces.unshift(record); await writeSpaces(spaces); return sendJson(res, existing ? 200 : 201, assetResponse(record)); }
    const spaceItem = url.pathname.match(/^\/api\/spaces\/([^/]+)$/);
    if (spaceItem && req.method === 'GET') { const actor = user(req); const space = (await readSpaces()).find((s) => s.spaceId === decodeURIComponent(spaceItem[1])); if (!space) return sendJson(res, 404, { success: false, message: 'Space not found' }); if (space.status !== 'published' && !canEditSpace(space, actor)) return sendJson(res, 403, { success: false, message: 'Forbidden' }); return sendJson(res, 200, assetResponse(space)); }
    if (spaceItem && req.method === 'DELETE') { const actor = user(req); const spaces = await readSpaces(); const space = spaces.find((s) => s.spaceId === decodeURIComponent(spaceItem[1])); if (!space) return sendJson(res, 404, { success: false, message: 'Space not found' }); if (!canEditSpace(space, actor)) return sendJson(res, 403, { success: false, message: 'Forbidden' }); await writeSpaces(spaces.filter((s) => s.spaceId !== space.spaceId)); return sendJson(res, 200, assetResponse({ spaceId: space.spaceId })); }
    const spaceAction = url.pathname.match(/^\/api\/spaces\/([^/]+)\/(publish|unpublish|duplicate)$/);
    if (spaceAction && req.method === 'POST') { const actor = user(req); const spaces = await readSpaces(); const space = spaces.find((s) => s.spaceId === decodeURIComponent(spaceAction[1])); if (!space) return sendJson(res, 404, { success: false, message: 'Space not found' }); if (!canEditSpace(space, actor)) return sendJson(res, 403, { success: false, message: 'Forbidden' }); if (spaceAction[2] === 'duplicate') { const copy = { ...space, spaceId: randomUUID(), name: cleanText(space.name + ' Copy'), status: 'draft', visibility: 'private', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), publishedAt: null }; spaces.unshift(copy); await writeSpaces(spaces); return sendJson(res, 201, assetResponse(copy)); } space.status = spaceAction[2] === 'publish' ? 'published' : 'draft'; space.visibility = space.status === 'published' ? 'public' : 'private'; space.updatedAt = new Date().toISOString(); space.publishedAt = space.status === 'published' ? space.updatedAt : null; await writeSpaces(spaces); return sendJson(res, 200, assetResponse(space)); }
