...
async function backpackFor(account) {
  ...
  await userDatabase.ensureAccount(account);
  const state = await userDatabase.reconcilePurchases(account, await paymentOrderStore.list());
  const purchased = (state.items || []).filter((item) => item.source === 'verified-purchase').map((item) => ({ ...item, state: 'revealed', assetType: item.assetType || (/avatar/i.test(`${item.category || ''} ${item.name || ''}`) ? 'avatar' : 'prop'), accessType: 'owned' }));
  const assets = [...(account.gameAssets || []).map((name) => stateAsset(name, 'game')), ...(account.purchasedAssets || []).map((name) => stateAsset(name, 'purchase')), ...purchased];
  ...
  return { accountId: account.accountId, backpackId: account.backpackId, status: account.loadoutRedeemed ? 'ready' : 'empty', mzkBalance: Number(state.tokens?.MZK || 0), land: (account.landAssets || []).map((name) => stateAsset(name, 'land')), avatars: [STARTER_AVATAR, ...catalogAvatars, ...assets.filter((asset) => asset.assetType === 'avatar' && asset.name !== 'Starter Avatar').map((asset) => ({ ...asset, eligible: asset.state === 'revealed' }))], assets, catalogAssets: catalog, environments: [...catalog.filter((asset) => asset.assetType === 'environment'), ...purchased.filter((asset) => asset.assetType === 'environment')], props: [...catalog.filter((asset) => asset.assetType === 'prop'), ...purchased.filter((asset) => asset.assetType === 'prop')], vehicles: [...catalog.filter((asset) => asset.assetType === 'vehicle'), ...purchased.filter((asset) => asset.assetType === 'vehicle')], bottleClaims: (account.bottleClaims || []).map((name) => stateAsset(name, 'bottle')), entitlements: [...(account.creatorVaultAccess ? ['creator-vault'] : []), ...(account.marketplaceAccess ? ['marketplace'] : []), ...(account.gameAccess ? ['members-game'] : [])], selectedAvatarId: account.selectedAvatarId || STARTER_AVATAR.id, updatedAt: state.updatedAt || account.updatedAt };
}
