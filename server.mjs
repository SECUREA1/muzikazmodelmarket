...
function requireLandOwnership(req, res) {
  ...
  const knownWorld = /\b(?:volt city|skyline deck|echo gardens|crew plaza|studio ridge|neon docks|bassline badlands|pixel peaks)\b/i.test(landAsset);
  if (landAsset && (knownWorld || /\b(?:muzikaz\s+world|digital\s+land|world\s+plot|land\s+(?:asset|world|reservation|deed|claim))\b/i.test(landAsset))) return landAsset;
...
function corsHeaders(extra = {}) { return { 'Access-Control-Allow-Origin': process.env.MUZIKAZ_CORS_ORIGIN || 'https://muzikazmodelmarket.onrender.com', 'Access-Control-Allow-Methods': 'GET, PUT, POST, PATCH, DELETE, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Wallet-Address, X-MUZIKAZ-Session, X-User-Id, X-User-Role, X-User-Name, X-Admin-Token, X-MUZIKAZ-Land-Asset', 'Cross-Origin-Resource-Policy': 'cross-origin', ...extra }; }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/payments/payouts' && req.method === 'POST') return sendJson(res, 200, assetResponse(await cryptoPayoutService.request(await bodyJson(req))));
    ...
    if (url.pathname === '/api/environments/upload' && req.method === 'POST') { if (!requireLandOwnership(req, res) || !requireAdmin(req, res)) return; return sendJson(res, 201, assetResponse(await saveEnvironmentUpload(req))); }
    ...
    if (url.pathname === '/api/assets/upload' && req.method === 'POST') { if (!requireLandOwnership(req, res) || !requireAdmin(req, res)) return; return sendJson(res, 201, assetResponse(await saveAssetUpload(req, false))); }
    if (url.pathname === '/api/models/upload' && req.method === 'POST') { if (!requireLandOwnership(req, res) || !requireAdmin(req, res)) return; return sendJson(res, 201, assetResponse(await saveAssetUpload(req, true))); }
    ...
    if (url.pathname === '/api/uploads/avatar' && req.method === 'POST') { if (!requireLandOwnership(req, res)) return; return sendJson(res, 201, await uploadFile(req)); }
