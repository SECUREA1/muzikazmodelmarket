...
function loadoutPermissions(account) {
  return {
    backpack: account.loadoutAccess === true,
    avatarSelection: account.loadoutAccess === true && account.avatarAccess === true,
    creatorTools: account.loadoutAccess === true && account.creatorVaultAccess === true,
    marketplace: account.loadoutAccess === true && account.marketplaceAccess === true,
    radTox: account.loadoutAccess === true && account.gameAccess === true,
    games: account.loadoutAccess === true && account.gameAccess === true,
    world: account.loadoutAccess === true && account.worldAccess === true
  };
}
...
async function canonicalMember(req, res, csrf = false) {
  const active = await accountSession(req, res, csrf); if (!active) return null;
  ...
  return { active, account, state, walletId: state.walletId };
}
...
async function entitledAccount(active, res) {
  ...
  await loadoutCodeStore.repairEntitledAccount(canonical.accountId);
  const repaired = await loadoutCodeStore.getAccount(canonical.accountId);
  if (repaired?.loadoutAccess !== true) { authorizationError(res, 403, 'LOADOUT_ACCESS_REQUIRED', 'This account does not have a Loadout entitlement.', 'entitlement'); return null; }
...
async function multiplayerContext(req, res) {
  const active = await accountSession(req, res); if (!active) return null;
  const account = await entitledAccount(active, res); if (!account) return null;
...
async function repositoryGameCatalog() {
  ...
  const worlds = environments.map((environment) => ({
    id: `environment-${environment.id}`, name: environment.name, assetType: 'environment', category: environment.category || environment.label || 'Environments', modelUrl: environment.modelUrl || environment.modelUrls?.[0] || '', modelUrls: environment.modelUrls, thumbnailUrl: environment.thumbnailUrl || '', playable: environment.playable !== false, levelOrder: environment.levelOrder, source: 'repository', accessType: 'included'
  }));
...
async function readPublicAvatars() { try { const catalog = await repositoryGameCatalog(); return catalog.filter((asset) => asset.assetType === 'avatar').map((asset) => ({ ...asset, id: asset.id.replace(/^repository-/, ''), creator: 'MUZIKAZ' })); } catch { try { return JSON.parse(await readFile(publicAvatarManifest, 'utf8')); } catch { return []; } } }
...
function session(req) { return String(req.headers['x-muzikaz-session'] || new URL(req.url, 'http://x').searchParams.get('sessionId') || req.muzikazAuthentication?.id || '').replace(/[^a-zA-Z0-9._-]/g, '').slice(0, 120) || randomUUID(); }
...
function corsHeaders(extra = {}, origin = '') {
  const configured = String(process.env.MUZIKAZ_CORS_ORIGINS || process.env.MUZIKAZ_CORS_ORIGIN || '').split(',').map((item) => item.trim()).filter(Boolean);
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/account/bootstrap' && req.method === 'GET') {
      ...
      return sendJson(res, 200, assetResponse({ authenticated: true, account: canonical, backpack: await backpackFor(canonical), permissions: loadoutPermissions(canonical), session: { mechanism: active.mechanism, createdAt: active.createdAt, expiresAt: active.expiresAt }, csrfToken, expiresAt: active.expiresAt }));
    }
    ...
    if (url.pathname === '/api/backpack' && req.method === 'GET') { const active = await accountSession(req, res); if (!active) return; const account = await loadoutCodeStore.getAccount(active.accountId); return account ? sendJson(res, 200, assetResponse(await backpackFor(account))) : authorizationError(res, 401, 'ACCOUNT_NOT_FOUND', 'Account not found.', 'account'); }
    if (url.pathname === '/api/profile' && req.method === 'GET') {
      const member = await canonicalMember(req, res); if (!member) return;
      const activity = await userDatabase.activity(member.walletId);
      return sendJson(res, 200, assetResponse({
        accountId: member.account.accountId, backpackId: member.account.backpackId,
        username: member.account.username, walletId: member.walletId,
        connectedWallets: member.account.connectedWallets || [], tokens: member.state.tokens,
        items: member.state.items, memory: member.state.memory, activity,
        selectedAvatarId: member.account.selectedAvatarId || STARTER_AVATAR.id,
        updatedAt: member.state.updatedAt
      }));
    }
    if (url.pathname === '/api/profile/memory' && req.method === 'PATCH') {
      const member = await canonicalMember(req, res, true); if (!member) return;
      const body = await bodyJson(req);
      const updated = await userDatabase.put(member.walletId, { memory: body.memory });
      return sendJson(res, 200, assetResponse({ memory: updated.memory, revision: updated.revision, updatedAt: updated.updatedAt }));
    }
    if (url.pathname === '/api/avatars' && req.method === 'GET') { const active = await accountSession(req, res); if (!active) return; const account = await loadoutCodeStore.getAccount(active.accountId); return sendJson(res, 200, assetResponse((await backpackFor(account)).avatars)); }
    if (url.pathname === '/api/avatar-selection' && req.method === 'PUT') { const active = await accountSession(req, res, true); if (!active) return; const requested = (await bodyJson(req)).avatarId; const current = await loadoutCodeStore.getAccount(active.accountId); if (!(await backpackFor(current)).avatars.some((avatar) => avatar.id === requested && avatar.eligible)) return authorizationError(res, 409, 'AVATAR_UNAVAILABLE', 'That avatar is not available in this game Backpack.', 'inventory'); const account = await loadoutCodeStore.selectAvatar(active.accountId, requested); return sendJson(res, 200, assetResponse({ selectedAvatarId: account.selectedAvatarId, updatedAt: account.updatedAt })); }
    if (url.pathname === '/api/game/session' && req.method === 'POST') {
      ...
      const backpack = await backpackFor(account); const avatar = backpack.avatars.find((item) => item.id === backpack.selectedAvatarId && item.eligible) || STARTER_AVATAR;
    ...
    if (url.pathname === '/api/game/session' && req.method === 'GET') {
      ...
      const backpack = await backpackFor(account); const selectedAvatar = backpack.avatars.find((item) => item.id === game.avatarId && item.eligible) || STARTER_AVATAR;
    ...
    if (url.pathname === '/api/admin/data' && req.method === 'GET') {
      ...
      const mzkTransactions = (usersData.transactions || []).slice();
      const backpackItems = users.flatMap(({ walletKey, record }) => (record.items || []).map((item) => ({ walletId: record.walletId || walletKey, itemId: item.id, name: item.name || item.title || item.id, record: item })));
      const items = [...backpackItems, ...submissions.map((item) => ({ itemId: item.id, name: item.title, record: item }))];
      const spaces = Object.values(usersData.landDeeds || {}).map((deed) => ({ spaceId: deed.id, worldId: deed.worldId, ownerId: deed.ownerId, record: deed }));
      ...
      return sendJson(res, 200, assetResponse({
        ...
        mzkTransactions, items, backpackItems, spaces, submissions, users, sales, models, customizations: [], derivatives: [], environments, avatars, avatarProfiles, gameplaySpending: gameplay.transactions
      }));
    }
    ...
    if (url.pathname === '/api/access/free-play' && req.method === 'POST') { const attempt = throttleAccess(req); try { const body = await bodyJson(req); const account = await loadoutCodeStore.freePlayLogin(body.username, body.password); await userDatabase.ensureAccount(account); const opened = await openAccountSession(res, account); attempt.success(); return sendJson(res, 200, assetResponse({ ...opened, backpack: await backpackFor(account), permissions: loadoutPermissions(account) })); } catch (error) { attempt.failure(); throw error; } }
    if ((url.pathname === '/api/access/username' || url.pathname === '/api/login') && req.method === 'POST') { const attempt = throttleAccess(req); try { const body = await bodyJson(req); const account = await loadoutCodeStore.usernameLogin(body.username); await userDatabase.ensureAccount(account); const opened = await openAccountSession(res, account); attempt.success(); return sendJson(res, 200, assetResponse({ ...opened, backpack: await backpackFor(account), permissions: loadoutPermissions(account) })); } catch (error) { attempt.failure(); throw error; } }
    ...
    if (url.pathname === '/api/account/avatar' && req.method === 'PUT') { const active = await accountSession(req, res, true); if (!active) return; const requested = (await bodyJson(req)).avatarId; const current = await loadoutCodeStore.getAccount(active.accountId); if (!(await backpackFor(current)).avatars.some((avatar) => avatar.id === requested && avatar.eligible)) return authorizationError(res, 409, 'AVATAR_UNAVAILABLE', 'That avatar is not available in this game Backpack.', 'inventory'); const account = await loadoutCodeStore.selectAvatar(active.accountId, requested); await userDatabase.ensureAccount(account); return sendJson(res, 200, assetResponse(account)); }
    ...
    if (url.pathname === '/api/market/listings' && req.method === 'PUT') { const member = await canonicalMember(req, res, true); if (!member) return; const body = await bodyJson(req); return sendJson(res, 200, assetResponse(await userDatabase.listItem(member.walletId, cleanText(body.itemId), body.priceMzk, body.active !== false))); }
    if (url.pathname === '/api/market/trades' && req.method === 'POST') { const member = await canonicalMember(req, res, true); if (!member) return; const body = await bodyJson(req); return sendJson(res, 201, assetResponse(await userDatabase.trade({ buyerId: member.walletId, sellerId: body.sellerId, itemId: body.itemId, requestId: body.requestId }))); }
    if (url.pathname === '/api/market/messages' && req.method === 'POST') { const member = await canonicalMember(req, res, true); if (!member) return; const body = await bodyJson(req); return sendJson(res, 201, assetResponse(await userDatabase.message({ from: member.walletId, to: body.to, text: body.text }))); }
    if (url.pathname === '/api/market/activity' && req.method === 'GET') { const member = await canonicalMember(req, res); if (!member) return; return sendJson(res, 200, assetResponse(await userDatabase.activity(member.walletId, url.searchParams.get('peer') || ''))); }
    ...
    if (url.pathname === '/api/houses/ioncore-house/presence' && req.method === 'POST') { const authentication = await resolveAccountSession(req); const account = authentication ? await loadoutCodeStore.getAccount(authentication.accountId) : null; const id = session(req); const actor = user(req); const body = await bodyJson(req).catch(() => ({})); const backpack = account ? await backpackFor(account) : null; const asset = backpack?.avatars.find((item) => item.id === backpack.selectedAvatarId && item.eligible) || STARTER_AVATAR; activePresence(); if (!presence.has(id) && presence.size >= maxHouseUsers) return sendJson(res, 409, { error: 'This Vibe Crib server is full.', ...presencePayload() }); const now = new Date().toISOString(); const previous = presence.get(id) || {}; const position = { x: clampNumber(body.position?.x, -4.65, 4.65, previous.position?.x || 0), y: clampNumber(body.position?.y, 0, 2.4, previous.position?.y || 0), z: clampNumber(body.position?.z, .35, 8.65, previous.position?.z || 2.5) }; const rotation = { x: 0, y: clampNumber(body.rotation?.y, -Math.PI * 4, Math.PI * 4, previous.rotation?.y || 0), z: 0 }; presence.set(id, { sessionId: id, userId: actor.id, username: account?.username || actor.name, joinedAt: previous.joinedAt || now, lastActiveAt: now, roomId: cleanText(body.roomId, roomId(position)), color: cleanText(body.color, previous.color || '#9cff00'), avatarAssetId: asset.id, modelUrl: asset.modelUrl, avatarName: asset.name, position, rotation, movementState: ['idle','walk','run','jump'].includes(body.movementState) ? body.movementState : 'idle', animationState: cleanText(body.animationState, 'auto'), message: cleanText(body.message, previous.message || ''), voiceEnabled: body.voiceEnabled === true }); const data = presencePayload(); broadcast('player-state', presence.get(id)); broadcast('house-presence-updated', data); return sendJson(res, 200, data); }
    ...
    if (url.pathname === '/api/houses/ioncore-house/voice/signal' && req.method === 'POST') { const from = session(req); const sender = presence.get(from); if (!sender) return sendJson(res, 401, { error: 'Join the Vibe Crib before using voice.' }); const body = await bodyJson(req); const to = cleanText(body.to, ''); const recipient = presence.get(to); if (!to || to === from || !recipient || recipient.roomId !== sender.roomId) return sendJson(res, 400, { error: 'Voice recipient is unavailable in this room.' }); const kind = ['offer','answer','candidate','hangup'].includes(body.kind) ? body.kind : ''; if (!kind) return sendJson(res, 400, { error: 'Unsupported voice signal.' }); broadcastTo(to, 'house-voice-signal', { from, to, kind, payload: body.payload || null }); return sendJson(res, 202, { ok: true }); }
    ...
    if (url.pathname === '/api/houses/ioncore-house/chat' && req.method === 'POST') { await resolveAccountSession(req); const id = session(req); const person = presence.get(id); if (!person) return sendJson(res, 401, { error: 'Join the Vibe Crib before chatting.' }); const body = await bodyJson(req); const message = cleanText(body.message, '').trim(); if (!message) return sendJson(res, 400, { error: 'Message cannot be empty.' }); const record = { id: randomUUID(), sessionId: id, username: person.username, roomId: person.roomId, message, createdAt: new Date().toISOString() }; person.message = message; person.lastActiveAt = record.createdAt; chatMessages.push(record); broadcast('house-presence-updated', presencePayload()); if (chatMessages.length > 100) chatMessages.splice(0, chatMessages.length - 100); broadcast('house-chat-message', record); return sendJson(res, 201, record); }
  ...
  } catch (error) { if (!res.headersSent) sendJson(res, error.statusCode || (url.pathname.startsWith('/api/') ? 500 : 404), { success: false, code: error.code || (url.pathname.startsWith('/api/') ? 'API_REQUEST_FAILED' : 'NOT_FOUND'), message: error.message || 'Not found', stage: error.stage || (error.statusCode === 409 ? 'account-state' : 'request') }); }
});
