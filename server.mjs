...
function openAccountSession(res, account) { const previous = cookie(res.muzikazRequest, 'mzk_session'); if (previous) accountSessions.delete(previous); const token = randomBytes(32).toString('base64url'); const csrfToken = randomBytes(24).toString('base64url'); const expiresAt = Date.now() + 8 * 60 * 60 * 1000; accountSessions.set(token, { accountId: account.accountId, wallet: account.primaryEthereumWallet, csrfToken, expiresAt }); res.setHeader('Set-Cookie', sessionCookie('mzk_session', token, 28800)); return { authenticated: true, account, csrfToken, expiresAt: new Date(expiresAt).toISOString() }; }
function optionalAccountSession(req) { const token = cookie(req, 'mzk_session'); const current = accountSessions.get(token); if (!current || current.expiresAt <= Date.now()) { if (token) accountSessions.delete(token); return null; } return current; }
function accountSession(req, res, csrf = false) { const token = cookie(req, 'mzk_session'); const current = accountSessions.get(token); if (!current || current.expiresAt <= Date.now()) { accountSessions.delete(token); sendJson(res, 401, { success: false, code: 'SESSION_REQUIRED', message: 'Your account session is missing or expired.' }); return null; } if (csrf) { const supplied = String(req.headers['x-csrf-token'] || ''); const a = Buffer.from(supplied); const b = Buffer.from(current.csrfToken); if (a.length !== b.length || !timingSafeEqual(a, b)) { sendJson(res, 403, { success: false, code: 'CSRF_INVALID', message: 'Invalid CSRF token.' }); return null; } } return current; }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/game/session' && req.method === 'POST') { const active = accountSession(req, res, true); if (!active) return; const account = await loadoutCodeStore.getAccount(active.accountId); const backpack = backpackFor(account); if (!account.gameAccess || backpack.status !== 'ready') return sendJson(res, 403, { success: false, code: 'GAME_ACCESS_REQUIRED', message: 'A ready Members Backpack is required.' }); const avatar = backpack.avatars.find((item) => item.id === backpack.selectedAvatarId && item.eligible) || STARTER_AVATAR; const token = randomBytes(32).toString('base64url'); const expiresAt = Date.now() + 5 * 60_000; gameSessions.set(token, { accountId: account.accountId, avatarId: avatar.id, expiresAt }); res.setHeader('Set-Cookie', sessionCookie('mzk_game', token, 300)); return sendJson(res, 201, assetResponse({ expiresAt: new Date(expiresAt).toISOString() })); }
