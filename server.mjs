...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/houses/ioncore-house/events' && req.method === 'GET') { const access = await multiplayerContext(req, res); if (!access) return; res.writeHead(200, corsHeaders({ 'Content-Type': 'text/event-stream', 'Cache-Control': 'no-cache', Connection: 'keep-alive' })); res.muzikazSessionId = session(req); res.write(`event: house-presence-updated\ndata: ${JSON.stringify(presencePayload())}\n\n`); clients.add(res); req.on('close', () => clients.delete(res)); return; }
    if (url.pathname === '/api/houses/ioncore-house/presence' && req.method === 'POST') { const access = await multiplayerContext(req, res); if (!access) return; const id = session(req); const actor = { id: access.account.accountId, name: access.account.username }; const body = await bodyJson(req).catch(() => ({})); const backpack = await backpackFor(access.account); const asset = backpack.avatars.find((item) => item.id === backpack.selectedAvatarId && item.eligible) || STARTER_AVATAR; activePresence(); if (!presence.has(id) && presence.size >= maxHouseUsers) return sendJson(res, 409, { error: 'This Vibe Crib server is full.', ...presencePayload() }); const now = new Date().toISOString(); const previous = presence.get(id) || {}; const position = { x: clampNumber(body.position?.x, -4.65, 4.65, previous.position?.x || 0), y: clampNumber(body.position?.y, 0, 2.4, previous.position?.y || 0), z: clampNumber(body.position?.z, .35, 8.65, previous.position?.z || 2.5) }; const rotation = { x: 0, y: clampNumber(body.rotation?.y, -Math.PI * 4, Math.PI * 4, previous.rotation?.y || 0), z: 0 }; presence.set(id, { sessionId: id, accountId: access.account.accountId, userId: actor.id, username: actor.name, joinedAt: previous.joinedAt || now, lastActiveAt: now, roomId: cleanText(body.roomId, roomId(position)), color: cleanText(body.color, previous.color || '#9cff00'), avatarAssetId: asset.id, modelUrl: asset.modelUrl, avatarName: asset.name, position, rotation, movementState: ['idle','walk','run','jump'].includes(body.movementState) ? body.movementState : 'idle', animationState: cleanText(body.animationState, 'auto'), message: cleanText(body.message, previous.message || '') }); const data = presencePayload(); broadcast('player-state', presence.get(id)); broadcast('house-presence-updated', data); return sendJson(res, 200, data); }
    if (url.pathname === '/api/houses/ioncore-house/presence/leave' && req.method === 'POST') { const access = await multiplayerContext(req, res); if (!access) return; const id = session(req); if (presence.get(id)?.accountId === access.account.accountId) presence.delete(id); const data = presencePayload(); broadcast('house-presence-updated', data); return sendJson(res, 200, { ok: true, ...data }); }
    if (url.pathname === '/api/houses/ioncore-house/voice/signal' && req.method === 'POST') { const access = await multiplayerContext(req, res); if (!access) return; const from = session(req); const sender = presence.get(from); if (!sender || sender.accountId !== access.account.accountId) return sendJson(res, 401, { error: 'Join the Vibe Crib before using voice.' }); const body = await bodyJson(req); const to = cleanText(body.to, ''); if (!to || to === from || !presence.has(to)) return sendJson(res, 400, { error: 'Voice recipient is unavailable.' }); const kind = ['offer','answer','candidate','hangup'].includes(body.kind) ? body.kind : ''; if (!kind) return sendJson(res, 400, { error: 'Unsupported voice signal.' }); broadcastTo(to, 'house-voice-signal', { from, to, kind, payload: body.payload || null }); return sendJson(res, 202, { ok: true }); }
    if (url.pathname === '/api/houses/ioncore-house/chat' && req.method === 'GET') { if (!await multiplayerContext(req, res)) return; return sendJson(res, 200, { messages: chatMessages.slice(-50) }); }
    if (url.pathname === '/api/houses/ioncore-house/chat' && req.method === 'POST') { const access = await multiplayerContext(req, res); if (!access) return; const id = session(req); const person = presence.get(id); if (!person || person.accountId !== access.account.accountId) return sendJson(res, 401, { error: 'Join the Vibe Crib before chatting.' }); const body = await bodyJson(req); const message = cleanText(body.message, '').trim(); if (!message) return sendJson(res, 400, { error: 'Message cannot be empty.' }); const record = { id: randomUUID(), sessionId: id, username: person.username, message, createdAt: new Date().toISOString() }; person.message = message; person.lastActiveAt = record.createdAt; chatMessages.push(record); broadcast('house-presence-updated', presencePayload()); if (chatMessages.length > 100) chatMessages.splice(0, chatMessages.length - 100); broadcast('house-chat-message', record); return sendJson(res, 201, record); }
