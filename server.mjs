...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/wallet/state' && req.method === 'PUT') { const member = await canonicalMember(req, res, true); if (!member) return; return authorizationError(res, 405, 'AUTHORITATIVE_STATE_REQUIRED', 'Backpack items and token balances can only be changed by verified purchases, marketplace trades, and game transactions. Use /api/profile/memory for profile saves.', 'state'); }
    ...
    if (url.pathname === '/api/houses/ioncore-house/chat' && req.method === 'GET') { const active = await accountSession(req, res); if (!active) return; const requestedRoom = cleanText(url.searchParams.get('roomId'), '').trim(); const roomMessages = requestedRoom ? chatMessages.filter((message) => message.roomId === requestedRoom) : chatMessages; return sendJson(res, 200, { messages: roomMessages.slice(-50) }); }
