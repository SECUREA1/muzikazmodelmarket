...
function backpackFor(account) {
  ...
  return { accountId: account.accountId, backpackId: account.backpackId, status: account.loadoutRedeemed ? 'ready' : 'empty', mzkBalance: Number(account.mzkBalance || 0), land: (account.landAssets || []).map((name) => stateAsset(name, 'land')), avatars: [STARTER_AVATAR, ...assets.filter((asset) => /avatar/i.test(asset.name) && asset.name !== 'Starter Avatar').map((asset) => ({ ...asset, eligible: asset.state === 'revealed' }))], assets, bottleClaims: (account.bottleClaims || []).map((name) => stateAsset(name, 'bottle')), entitlements: [...(account.loadoutAccess ? ['backpack-loadout'] : []), ...(account.creatorVaultAccess ? ['creator-vault'] : []), ...(account.marketplaceAccess ? ['marketplace'] : []), ...(account.gameAccess ? ['members-game'] : [])], selectedAvatarId: account.selectedAvatarId || STARTER_AVATAR.id, updatedAt: account.updatedAt };
}
