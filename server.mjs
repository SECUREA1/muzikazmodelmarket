...
function openAccountSession(res, account) { const previous = cookie(res.muzikazRequest, 'mzk_session'); if (previous) accountSessions.delete(previous); const token = randomBytes(32).toString('base64url'); const csrfToken = randomBytes(24).toString('base64url'); const expiresAt = Date.now() + 8 * 60 * 60 * 1000; accountSessions.set(token, { accountId: account.accountId, wallet: account.primaryEthereumWallet, csrfToken, expiresAt: Date.now() + 15 * 60_000 }); const cookieToken = randomBytes(32).toString('base64url'); accountSessions.set(cookieToken, { accountId: account.accountId, wallet: account.primaryEthereumWallet, csrfToken, expiresAt }); res.setHeader('Set-Cookie', sessionCookie('mzk_session', cookieToken, 28800)); return { authenticated: true, account, csrfToken, portableSession: token, expiresAt: new Date(expiresAt).toISOString(), portableExpiresAt: new Date(Date.now() + 15 * 60_000).toISOString() }; }
function sessionToken(req) { const authorization = String(req.headers.authorization || ''); const match = authorization.match(/^Bearer\s+([A-Za-z0-9_-]+)$/i); return match ? { token: match[1], bearer: true } : { token: cookie(req, 'mzk_session'), bearer: false }; }
function optionalAccountSession(req) { const selected = sessionToken(req); const current = accountSessions.get(selected.token); if (!current || current.expiresAt <= Date.now()) { if (selected.token) accountSessions.delete(selected.token); return null; } return { ...current, bearer: selected.bearer, token: selected.token }; }
function accountSession(req, res, csrf = false) { const current = optionalAccountSession(req); if (!current) { sendJson(res, 401, { success: false, code: 'SESSION_REQUIRED', message: 'Your account session is missing or expired.' }); return null; } if (csrf && !current.bearer) { const supplied = String(req.headers['x-csrf-token'] || ''); const a = Buffer.from(supplied); const b = Buffer.from(current.csrfToken); if (a.length !== b.length || !timingSafeEqual(a, b)) { sendJson(res, 403, { success: false, code: 'CSRF_INVALID', message: 'Invalid CSRF token.' }); return null; } } return current; }
...
function backpackFor(account) {
  ...
  return { accountId: account.accountId, backpackId: account.backpackId, status: account.loadoutRedeemed ? 'ready' : 'empty', mzkBalance: Number(account.mzkBalance || 0), land: (account.landAssets || []).map((name) => stateAsset(name, 'land')), avatars: [STARTER_AVATAR, ...assets.filter((asset) => /avatar/i.test(asset.name) && asset.name !== 'Starter Avatar').map((asset) => ({ ...asset, eligible: asset.state === 'revealed' }))], assets, bottleClaims: (account.bottleClaims || []).map((name) => stateAsset(name, 'bottle')), entitlements: [...(account.creatorVaultAccess ? ['creator-vault'] : []), ...(account.marketplaceAccess ? ['marketplace'] : []), ...(account.gameAccess ? ['members-game'] : [])], selectedAvatarId: account.selectedAvatarId || STARTER_AVATAR.id, updatedAt: account.updatedAt };
}
function bootstrapFor(account, session) { const backpack = backpackFor(account); const entitled = Boolean(account.loadoutAccess || account.loadoutRedeemed); return { authenticated: true, accountId: account.accountId, boundWallets: account.connectedWallets || [], session: { mechanism: session.bearer ? 'bearer' : 'cookie', expiresAt: new Date(session.expiresAt).toISOString() }, loadout: { entitled, provisioningVersion: Number(account.loadoutProvisioningVersion || 0), status: account.loadoutStatus }, backpack, mzkBalance: backpack.mzkBalance, land: backpack.land, bottleClaims: backpack.bottleClaims, permissions: { members: entitled, creator: entitled && Boolean(account.creatorVaultAccess), radTox: entitled && Boolean(account.gameAccess) }, selectedAvatar: backpack.avatars.find((item) => item.id === backpack.selectedAvatarId && item.eligible) || STARTER_AVATAR }; }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/session' && req.method === 'GET') { const active = accountSession(req, res); if (!active) return; const account = await loadoutCodeStore.getAccount(active.accountId); if (!account) return sendJson(res, 401, { success: false, code: 'ACCOUNT_NOT_FOUND', message: 'The session account no longer exists.' }); return sendJson(res, 200, assetResponse({ authenticated: true, accountId: account.accountId, backpackId: account.backpackId, csrfToken: active.csrfToken, expiresAt: new Date(active.expiresAt).toISOString() })); }
    if ((url.pathname === '/api/bootstrap' || url.pathname === '/api/account/bootstrap') && req.method === 'GET') { const active = accountSession(req, res); if (!active) return; const account = await loadoutCodeStore.repairEntitledAccount(active.accountId); await userDatabase.ensureAccount(account); return sendJson(res, 200, assetResponse(bootstrapFor(account, active))); }
    if (url.pathname === '/api/session' && req.method === 'DELETE') { const active = accountSession(req, res, true); if (!active) return; accountSessions.delete(active.token); res.setHeader('Set-Cookie', sessionCookie('mzk_session', '', 0)); return sendJson(res, 200, assetResponse({ authenticated: false })); }
    ...
    if (url.pathname === '/api/game/session' && req.method === 'POST') { const active = accountSession(req, res, true); if (!active) return; const account = await loadoutCodeStore.repairEntitledAccount(active.accountId); const backpack = backpackFor(account); if (!(account.loadoutAccess || account.loadoutRedeemed) || !account.gameAccess || backpack.status !== 'ready') return sendJson(res, 403, { success: false, code: 'GAME_ACCESS_REQUIRED', message: 'A ready Members Backpack is required.' }); const avatar = backpack.avatars.find((item) => item.id === backpack.selectedAvatarId && item.eligible) || STARTER_AVATAR; const token = randomBytes(32).toString('base64url'); const expiresAt = Date.now() + 5 * 60_000; gameSessions.set(token, { accountId: account.accountId, avatarId: avatar.id, expiresAt }); res.setHeader('Set-Cookie', sessionCookie('mzk_game', token, 300)); return sendJson(res, 201, assetResponse({ expiresAt: new Date(expiresAt).toISOString() })); }
    if (url.pathname === '/api/game/session' && req.method === 'GET') { const active = accountSession(req, res); if (!active) return; const game = gameSessions.get(cookie(req, 'mzk_game')); if (!game || game.expiresAt <= Date.now() || game.accountId !== active.accountId) return sendJson(res, 401, { success: false, code: 'GAME_SESSION_REQUIRED', message: 'Create a new game session from Members.' }); const account = await loadoutCodeStore.getAccount(active.accountId); const backpack = backpackFor(account); const selectedAvatar = backpack.avatars.find((item) => item.id === game.avatarId && item.eligible) || STARTER_AVATAR; return sendJson(res, 200, assetResponse({ accountId: account.accountId, backpack, selectedAvatar, warning: selectedAvatar.id !== game.avatarId ? 'The selected model is unavailable; Starter Avatar was loaded.' : null, expiresAt: new Date(game.expiresAt).toISOString() })); }
  ...
  } catch (error) { if (!res.headersSent) sendJson(res, error.statusCode || (url.pathname.startsWith('/api/') ? 400 : 404), { success: false, code: error.code || (url.pathname.startsWith('/api/') ? 'API_REQUEST_FAILED' : 'NOT_FOUND'), message: error.message || 'Not found' }); }
});
