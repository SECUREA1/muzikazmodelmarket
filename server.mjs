...
function corsHeaders(extra = {}) { return { 'Access-Control-Allow-Origin': process.env.MUZIKAZ_CORS_ORIGIN || '*', 'Access-Control-Allow-Methods': 'GET, PUT, POST, PATCH, DELETE, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Wallet-Address, X-MUZIKAZ-Session, X-User-Id, X-User-Role, X-User-Name, X-Admin-Token, X-MUZIKAZ-Land-Asset', 'Cross-Origin-Resource-Policy': 'cross-origin', ...extra }; }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/account/wallet' && req.method === 'POST') { const active = accountSession(req, res, true); if (!active) return; return sendJson(res, 200, assetResponse(await loadoutCodeStore.connectWallet(active.accountId, (await bodyJson(req)).wallet))); }
