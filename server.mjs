...
async function multiplayerContext(req, res) {
  ...
  if (!account) { authorizationError(res, 401, 'ACCOUNT_NOT_FOUND', 'The session account no longer exists.', 'account'); return null; }
  const ownsGenieBottle = [...(account.bottleClaims || []), ...(account.bottleNFTs || [])]
    .some((claim) => /genie|wish bottle/i.test(String(claim?.name || claim?.title || claim)));
  const hasMemberGameAccess = account.memberAccess === true && account.loadoutAccess === true && account.gameAccess === true;
  if (!hasMemberGameAccess && !ownsGenieBottle) {
    authorizationError(res, 402, 'MULTIPLAYER_ACCESS_REQUIRED', 'Multiplayer requires paid member access or a Genie Bottle held by this account.', 'paywall');
    return null;
  }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/access/guest' && req.method === 'POST') { const attempt = throttleAccess(req); try { const body = await bodyJson(req); const account = await loadoutCodeStore.guestLogin(body.email, body.password); await userDatabase.ensureAccount(account); attempt.success(); return sendJson(res, 200, assetResponse(await openAccountSession(res, account))); } catch (error) { attempt.failure(); throw error; } }
