(function () {
  ...
  async function requestPayout(usd, currency, wallet, mzk) { return api('/api/payments/payouts', { method: 'POST', body: JSON.stringify({ usd, currency, wallet, mzk }) }); }
  async function quoteEthValue(eth, currency) { const liveRates = await rates(); return quote(Number(eth) * liveRates.ETH, currency); }
  window.MuzikazWalletPayments = { NETWORKS, RECIPIENTS, rates, quote, quoteEthValue, createOrder, submitOrder, initiate, pay, requestPayout, paymentUri };
}());
