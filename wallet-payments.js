(function () {
  ...
  async function requestPayout(payout) { return api('/api/payments/payouts', { method: 'POST', body: JSON.stringify(payout) }); }
  function evmProvider() { const injected = window.ethereum; return injected?.providers?.find((provider) => provider.isMetaMask) || (injected?.isMetaMask ? injected : null); }
  ...
  async function sendEvm(q) { const provider = evmProvider(); if (!provider?.request) return openWallet(q, 'MetaMask'); const network = NETWORKS[q.currency]; await switchEvm(provider, network); const [address] = await provider.request({ method: 'eth_requestAccounts' }); const value = BigInt(Math.ceil(q.amount * 1e9)) * 1000000000n; const transactionHash = await provider.request({ method: 'eth_sendTransaction', params: [{ from: address, to: q.recipient, value: `0x${value.toString(16)}` }] }); return { address, transactionHash, walletName: 'MetaMask' }; }
  ...
  async function initiate(q) { const network = NETWORKS[q.currency]; if (network.type === 'evm') return sendEvm(q); if (network.type === 'solana') return (window.phantom?.solana || window.solana?.isPhantom) ? sendSolana(q) : openWallet(q, 'Phantom'); if (network.type === 'cardano') return window.cardano?.lace?.enable ? sendCardano(q) : openWallet(q, 'Lace'); return openWallet(q, `${network.name} wallet`); }
  async function pay(usd, currency, product = {}) { const q = await quote(usd, currency); const order = await createOrder(q, product); const sent = await initiate(q); if (sent.opened) throw new Error(`${sent.walletName} opened. Complete payment there, then return to checkout and submit the transaction hash / ID for verification.`); const verified = await submitOrder(order.orderId, sent.transactionHash); if (!['PAID', 'FULFILLED'].includes(verified.paymentStatus)) throw new Error(`Transaction submitted for ${verified.paymentNetwork}. Status: ${verified.paymentStatus}. Assets and MZK remain locked until independent verification.`); return { ...q, ...sent, orderId: order.orderId, paymentStatus: verified.paymentStatus }; }
  async function quoteEthValue(eth, currency) { const liveRates = await rates(); return quote(Number(eth) * liveRates.ETH, currency); }
  window.MuzikazWalletPayments = { NETWORKS, RECIPIENTS, rates, quote, quoteEthValue, createOrder, submitOrder, requestPayout, initiate, pay, paymentUri };
}());
