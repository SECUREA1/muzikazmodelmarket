...
(function () {
  ...
  function fail(stage, message){ clearWatchdog(); var screen=document.getElementById('house-game-start'); if(screen){screen.className=screen.className.replace(/\bis-hidden\b/g,'');screen.setAttribute('aria-hidden','false');} setState('error', stage+': '+message); var controls=buttons(); for(var i=0;i<controls.length;i+=1){controls[i].disabled=false; controls[i].textContent=controls[i].id==='house-start-game'?'Begin':'Start RAD-TOX';} addRecovery(); }
