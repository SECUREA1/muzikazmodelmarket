...
(function () {
  ...
  // The fallback is used on both desktop and mobile when the 3D engine is unavailable.
  // Keep it as a real multi-level game: completing a field must always stage the next one.
  function startCompatibility(){ clearWatchdog(); setState('compatibility-mode','Compatibility Mode active. Clear every toxic bubble.'); var stage=document.querySelector('.house-stage'), screen=document.getElementById('house-game-start'); if(!stage || stage.querySelector('.rad-tox-compat-game'))return; safeScrollIntoView(stage); var game=document.createElement('section'); game.className='rad-tox-compat-game'; game.innerHTML='<div class="rad-tox-compat-head"><strong>☢ RAD-TOX — Compatibility Mode</strong><span data-score>Level 1 · Clear 0 / 12</span></div><p data-level-message>Fast 2D mission for browsers without the 3D engine.</p><div class="rad-tox-compat-field"></div><button type="button" data-restart>Restart level</button>'; stage.appendChild(game);if(screen)screen.className+=' is-hidden';var level=1,score=0,advancing=false,field=game.querySelector('.rad-tox-compat-field'),label=game.querySelector('[data-score]'),message=game.querySelector('[data-level-message]'),audio;function targetCount(){return Math.min(12+(level-1)*4,28);}function updateLabel(){label.innerHTML='Level '+level+' · Clear '+score+' / '+targetCount();}function sound(kind){var A=window.AudioContext||window.webkitAudioContext,now,o,g;if(!A)return;try{audio=audio||new A();if(audio.state==='suspended')audio.resume();now=audio.currentTime;o=audio.createOscillator();g=audio.createGain();o.type=kind==='clear'?'triangle':'square';o.frequency.setValueAtTime(kind==='clear'?660:210+score*18,now);o.frequency.exponentialRampToValueAtTime(kind==='clear'?1320:420+score*22,now+(kind==='clear'?.2:.07));g.gain.setValueAtTime(.0001,now);g.gain.exponentialRampToValueAtTime(.07,now+.01);g.gain.exponentialRampToValueAtTime(.0001,now+(kind==='clear'?.24:.1));o.connect(g);g.connect(audio.destination);o.start(now);o.stop(now+.28);}catch(ignore){}}function populate(){advancing=false;score=0;field.innerHTML='';updateLabel();message.innerHTML='Level '+level+' is live. Clear every toxic bubble to unlock level '+(level+1)+'.';for(var i=0,count=targetCount();i<count;i+=1){var x=document.createElement('button');x.type='button';x.className='rad-tox-compat-bubble';x.style.left=(5+(i*29)%84)+'%';x.style.top=(10+(i*37)%70)+'%';x.innerHTML='☢';x.onclick=function(){if(this.disabled||advancing)return;this.disabled=true;this.setAttribute('data-popped','');this.innerHTML='✓';score+=1;sound(score===targetCount()?'clear':'pop');updateLabel();if(score===targetCount()){advancing=true;message.innerHTML='Level '+level+' clear! Loading level '+(level+1)+'…';status('Level '+level+' clear! Loading level '+(level+1)+'…');window.setTimeout(function(){level+=1;populate();status('Level '+level+' is active. Clear every toxic bubble to continue.');},650);}else status('Level '+level+': '+(targetCount()-score)+' toxic bubbles remain.');};field.appendChild(x);}} game.querySelector('[data-restart]').onclick=populate;populate();}
