...
(function () {
  ...
  var state = 'booting'; var queued = true; var watchdog = 0; var engineRequested = false; var modernSupport;
  ...
  function uncoverGame(){ var screen=document.getElementById('house-game-start'); if(!screen)return; screen.hidden=true; screen.setAttribute('aria-hidden','true'); if((' '+screen.className+' ').indexOf(' is-hidden ')<0)screen.className+=' is-hidden'; }
  ...
  // Never leave the world behind a launch cover. IE starts its lightweight game
  // synchronously; modern browsers begin fetching and opening the GLB world now.
  uncoverGame();
  if(isInternetExplorer) document.documentElement.className += ' is-internet-explorer';
  if(!supportsModern()) startCompatibility();
  else { setState('idle','Opening the RAD-TOX world automatically…'); request(); }
}());
