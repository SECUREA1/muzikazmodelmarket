...
(function () {
  ...
  function fail(stage, message){ clearWatchdog(); setState('error', stage+': '+message); var controls=buttons(); for(var i=0;i<controls.length;i+=1){controls[i].disabled=false; controls[i].textContent='START FLOOR SWEEP';} addRecovery(); }
  ...
  function request(){
    ...
    queued=true;
    setButtons(true,'Preparing IonCore…');
  ...
  document.addEventListener('click',function(e){var t=e.target;while(t&&t.id!=='house-fullscreen')t=t.parentNode;if(!t)return;e.preventDefault();toggleFullscreen();},true);
  document.addEventListener('fullscreenchange',syncFullscreen);document.addEventListener('webkitfullscreenchange',syncFullscreen);document.addEventListener('MSFullscreenChange',syncFullscreen);
  // Match the desktop app: reveal the loading state and start the best available
  // game immediately. IE receives the lightweight compatibility mission while
  // modern browsers begin fetching the 3D engine without an extra click.
  function autoStart(){queued=true;setButtons(true,'Preparing IonCore…');if(!supportsModern()){window.setTimeout(startCompatibility,0);return;}setState('booting','Loading the 3D engine. Your game will begin automatically…');startEngine();armWatchdog();}
