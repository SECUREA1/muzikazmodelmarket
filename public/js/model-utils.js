...
export function publicShareUrl(model){const url=new URL('model-explorer.html',window.location.origin);const current=new URLSearchParams(window.location.search);for(const key of ['environment','house']){const value=current.get(key);if(value)url.searchParams.set(key,value);}url.searchParams.set('model',model.id);url.searchParams.set('view','ar');return url.href;}
