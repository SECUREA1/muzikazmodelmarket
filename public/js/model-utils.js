...
export function openHomepageToAvatars(){if(location.pathname.endsWith('/')||location.pathname.endsWith('/index.html')||location.pathname.split('/').pop()===''){if(!location.hash){history.replaceState(null,'',normalizeLiveModelsUrl());requestAnimationFrame(()=>document.getElementById('doll-kit-avatars')?.scrollIntoView({block:'start'}));}}}
