...
export function normalizeLiveModelsUrl(){return `${window.location.origin}${window.location.pathname}#character-world-avatars`;}
export function normalizeLiveModelsHash(){if(location.hash==='#live-models'||(location.hash&&location.hash.includes('#character-world-avatars')&&location.hash!=='#character-world-avatars')) history.replaceState(null,'',normalizeLiveModelsUrl());}
export function openHomepageToAvatars(){if(location.pathname.endsWith('/')||location.pathname.endsWith('/index.html')||location.pathname.split('/').pop()===''){if(!location.hash){history.replaceState(null,'',normalizeLiveModelsUrl());requestAnimationFrame(()=>document.getElementById('character-world-avatars')?.scrollIntoView({block:'start'}));}}}
