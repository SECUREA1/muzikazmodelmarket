...
import { ENVIRONMENT_LIST_UPDATED_EVENT, logEnvironment } from './environments/environment-api.js';
...
if (legacyCanvas instanceof HTMLCanvasElement && stage && hud) {
  ...
  // Keep the original directional keypad intact: movement must never depend on
  // a menu button being present, and every visible button maps to one action.
  ...
  style.textContent = `
    ...
    .mobile-move-pad.mobile-move-pad-glb{width:100%;max-width:720px;margin:0 auto;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));grid-template-areas:'side-left forward side-right reset' 'zoom-in zoom-out reverse jump' 'avatar environment . .';gap:clamp(8px,1.8vw,14px);padding:clamp(10px,2vw,16px);border:1px solid rgba(156,255,0,.42);border-radius:24px;background:linear-gradient(180deg,rgba(6,10,5,.96),rgba(0,0,0,.88));box-shadow:0 18px 42px rgba(0,0,0,.45),0 0 28px rgba(156,255,0,.12);box-sizing:border-box;backdrop-filter:blur(14px)}
    .mobile-move-pad.mobile-move-pad-glb [data-mobile-area=side-left]{grid-area:side-left}.mobile-move-pad.mobile-move-pad-glb [data-mobile-area=forward]{grid-area:forward}.mobile-move-pad.mobile-move-pad-glb [data-mobile-area=side-right]{grid-area:side-right}.mobile-move-pad.mobile-move-pad-glb [data-mobile-area=reset]{grid-area:reset}.mobile-move-pad.mobile-move-pad-glb [data-mobile-area=zoom-in]{grid-area:zoom-in}.mobile-move-pad.mobile-move-pad-glb [data-mobile-area=zoom-out]{grid-area:zoom-out}.mobile-move-pad.mobile-move-pad-glb [data-mobile-area=reverse]{grid-area:reverse}.mobile-move-pad.mobile-move-pad-glb [data-mobile-area=jump]{grid-area:jump}.mobile-move-pad.mobile-move-pad-glb [data-mobile-area=avatar]{grid-area:avatar}.mobile-move-pad.mobile-move-pad-glb [data-mobile-area=environment]{grid-area:environment}
    ...
    .mobile-move-pad.mobile-move-pad-glb [data-mobile-area=jump],.mobile-move-pad.mobile-move-pad-glb [data-mobile-area=reset],.mobile-move-pad.mobile-move-pad-glb [data-mobile-area=avatar],.mobile-move-pad.mobile-move-pad-glb [data-mobile-area=environment]{background:linear-gradient(180deg,#fff,#e7e7e7);color:#071007}
  ...
  const radTox = { enabled: readRadToxEnabled(), active: false, score: 0, hp: 100, maxHp: 100, hits: 0, spawnSeed: 0, hurtCooldownUntil: 0, completed: false, settings: { targetCount: mobileQualityMode ? 16 : 26, speed: 1, bubbleHealth: 4, contactDamage: 6, maxHp: 100, points: 1, sound: true, damageEnabled: true } };
  ...
  let viewActive = true; let lastFrameTime = 0; let nextFootstepAt = 0; const targetFrameMs = mobileQualityMode || reducedMotion ? 1000 / 30 : 0;
  ...
  function unlockRadToxAudio() { gameAudio.setEnabled(radTox.settings.sound); gameAudio.unlock(); }
  ...
  function popToxicBubble(bubble) { if (!radTox.active || !radTox.hp || !bubble || bubble.userData.popped) return false; radTox.hits += 1; bubble.userData.health -= 1; if (bubble.userData.health > 0) { bubble.children.forEach((child) => { if (child.material?.emissive) child.material.emissiveIntensity = 2.4; }); window.setTimeout(() => bubble.children.forEach((child) => { if (child.material?.emissive) child.material.emissiveIntensity = 1.35; }), 90); gameAudio.effect('hit'); updateRadToxHud(); return true; } bubble.userData.popped = true; radTox.score += Number(radTox.settings.points); gameAudio.effect('pop'); const burst = new THREE.Points(new THREE.BufferGeometry().setFromPoints(Array.from({ length: 32 }, (_, index) => new THREE.Vector3(Math.sin(index * 2.4) * .32, Math.cos(index * 1.7) * .32, Math.cos(index * 3.1) * .32))), new THREE.PointsMaterial({ color: 0xc8ff4a, size: .1, transparent: true })); burst.position.copy(bubble.position); burst.userData.life = .62; toxicBubbleGroup.add(burst); toxicBursts.push(burst); toxicBubbleGroup.remove(bubble); updateRadToxHud(); if (!toxicBubbles.some((item) => !item.userData.popped)) { radTox.completed = true; gameAudio.effect('ready'); setStatus(`Map cleared — ${radTox.score} points secured. Restart with settings to play again.`); } return true; }
  function activateRadTox({ unlockAudio = true } = {}) { if (!radTox.enabled) return; if (unlockAudio) unlockRadToxAudio(); gameAudio.effect('ready'); radTox.active = true; radTox.score = 0; radTox.hits = 0; radTox.maxHp = Number(radTox.settings.maxHp); radTox.hp = radTox.maxHp; radTox.completed = false; radTox.spawnSeed += 1; createToxicBubbles(); setStatus('RAD-TOX is live: pop each fully 3D toxic bubble until its health reaches zero. Keep clear of targets to protect HP.'); }
  ...
  resetButton?.addEventListener('click', () => { unlockRadToxAudio(); gameAudio.effect('reset'); resetPlayer(); });
  ...
  radToxSettings.querySelectorAll('[data-radtox-setting]').forEach((control) => control.addEventListener('input', () => { const key = control.dataset.radtoxSetting; radTox.settings[key] = control.type === 'checkbox' ? control.checked : Number(control.value); if (key === 'sound') gameAudio.setEnabled(radTox.settings.sound); gameAudio.effect('click'); syncRadToxSettings(); saveRadToxSettings(); }));
  ...
  updateRadToxHud();
  document.querySelectorAll('[data-mobile-move]').forEach((oldButton) => { const button = oldButton.cloneNode(true); oldButton.replaceWith(button); const direction = button.dataset.mobileMove; const mobileLabel = { forward: '▲ Forward', back: '▼ Reverse', left: '◀ Side left', right: 'Side right ▶', jump: '⤴ Jump' }[direction]; if (mobileLabel) button.setAttribute('aria-label', mobileLabel); const begin = (e) => { e.preventDefault(); unlockRadToxAudio(); if (direction === 'jump') { if (player.onGround) { gameAudio.effect('jump'); player.velocity.y = player.jumpVelocity; player.onGround = false; } button.classList.add('is-active'); return; } mobile.add(direction); button.classList.add('is-active'); }; const end = (e) => { e.preventDefault(); if (direction !== 'jump') mobile.delete(direction); button.classList.remove('is-active'); }; button.addEventListener('pointerdown', begin); button.addEventListener('pointerup', end); button.addEventListener('pointercancel', end); button.addEventListener('pointerleave', end); });
  document.querySelectorAll('[data-mobile-zoom]').forEach((oldButton) => { const button = oldButton.cloneNode(true); oldButton.replaceWith(button); button.addEventListener('click', (e) => { e.preventDefault(); unlockRadToxAudio(); gameAudio.effect('click'); applyZoom(button.dataset.mobileZoom === 'in' ? -1 : 1); }); });
  ...
  window.addEventListener(ENVIRONMENT_LIST_UPDATED_EVENT, (event) => {
    const worlds = registry.replace(event.detail || []);
    warmEnvironmentModels(worlds);
    renderPicker();
  });
  ...
  if (startEnvironment) {
    ...
    try { await openHouseMap(); window.MuzikazHouseExplorer = { envLoader, player, renderer, scene }; }
    catch (error) { console.error('[MUZIKAZ House Explorer bootstrap]', error); walkButton.disabled = true; setStatus(`House startup failed: ${error.message || 'unknown error'}. See the browser console for the full diagnostic.`); }
  } else {
