...
if (legacyCanvas instanceof HTMLCanvasElement && stage && hud) {
  ...
  const library = document.createElement('div'); library.className = 'house-environment-library'; library.innerHTML = '<strong>Environment Library</strong><small>Loading server-backed GLB worlds…</small>';
  hud.querySelector('.house-actions')?.after(library);
  const uploadPanel = createEnvironmentUploadPanel({ setStatus, onUploaded: async (record, loadAfter) => { await refreshLibrary(); if (loadAfter) await loadById(record.id); } }); library.after(uploadPanel);
  ...
  const viewControls = document.createElement('div'); viewControls.className = 'house-view-controls'; viewControls.innerHTML = '<strong>View controls <output>Zoom 40%</output></strong><button type="button" data-zoom="in" aria-label="Zoom in">Zoom +</button><button type="button" data-zoom="out" aria-label="Zoom out">Zoom −</button><button type="button" data-scroll-toggle class="toggle-active" aria-pressed="true">Scroll zoom on</button>';
  ...
  let currentSpaceScale = 2.5; let scrollZoomEnabled = true;
  let playerCollider = new Capsule(new THREE.Vector3(0, player.radius, 2), new THREE.Vector3(0, player.height, 2), player.radius); let dragPointer = null; let turnReady = true; let activeEnvironment = null;
  ...
  function resetPlayer(spawn = player.spawn, rotationY = player.yaw) { player.spawn.copy(spawn); player.velocity.set(0,0,0); player.yaw = rotationY || 0; player.pitch = 0; player.eyeHeight = player.height; playerCollider = new Capsule(new THREE.Vector3(spawn.x, spawn.y + player.radius, spawn.z), new THREE.Vector3(spawn.x, spawn.y + player.height, spawn.z), player.radius); playerRig.position.copy(spawn); playerRig.rotation.set(0, player.yaw, 0); camera.position.set(0, player.eyeHeight, 0); camera.rotation.set(0,0,0); }
  async function loadById(id) { const env = registry.find(id) || registry.all()[0]; if (!env) return; env.spaceScale = currentSpaceScale; activeEnvironment = env; loadingMeter.hidden = false; setStatus(`Loading ${env.name} as a complete GLB world…`); try { const result = await envLoader.load(env); if (!result) return; resetPlayer(result.spawn.position, result.spawn.rotationY); scaleControl.querySelector('input').value = currentSpaceScale.toFixed(1); scaleControl.querySelector('output').textContent = `${currentSpaceScale.toFixed(1)}x`; loadingMeter.hidden = true; setStatus(`Ready: ${env.name}. Collision meshes: ${result.collision.collisionMeshes.length}; embedded lights: ${result.quality.embeddedLights}.`); const url = new URL(location.href); url.searchParams.set('environment', env.id); url.searchParams.set('house', env.id); history.replaceState({}, '', url); renderLibrary(); logEnvironment('Loaded world', { id: env.id, source: env.source }); } catch (error) { console.error('[MUZIKAZ Environment]', error); loadingMeter.hidden = true; setStatus(error.message || `Unable to load ${env.name}.`); } }
  function renderLibrary() { library.innerHTML = '<strong>Environment Library</strong>'; registry.all().forEach((env) => { const card = document.createElement('article'); card.className = 'environment-card'; if (activeEnvironment?.id === env.id) card.setAttribute('aria-current', 'true'); const size = env.fileSize ? `${(env.fileSize / 1048576).toFixed(1)} MB` : 'repository GLB'; card.innerHTML = `<strong>${env.name}</strong><small>${env.description || ''}</small><small>${env.source} · ${size} · ${env.collisionMode || 'auto'} collision</small><button type="button">Load</button>`; card.querySelector('button').addEventListener('click', () => loadById(env.id)); if (env.source === 'uploaded') { const del = document.createElement('button'); del.type = 'button'; del.className = 'danger'; del.textContent = 'Delete'; del.addEventListener('click', async () => { if (!confirm(`Delete ${env.name}?`)) return; await registry.delete(env.id); renderLibrary(); }); card.append(del); } library.append(card); }); }
  async function refreshLibrary() { try { await registry.refresh(); renderLibrary(); } catch (error) { setStatus(error.message); library.innerHTML = `<strong>Environment Library</strong><small>${error.message}</small>`; } }
  ...
  scaleControl.querySelector('input').addEventListener('input', (e) => applySpaceScale(e.target.value)); scaleControl.querySelectorAll('[data-space-scale]').forEach((button) => button.addEventListener('click', () => applySpaceScale(currentSpaceScale + (button.dataset.spaceScale === 'up' ? .1 : -.1)))); viewControls.querySelectorAll('[data-zoom]').forEach((button) => button.addEventListener('click', () => applyZoom(button.dataset.zoom === 'in' ? -1 : 1))); viewControls.querySelector('[data-scroll-toggle]').addEventListener('click', (e) => { scrollZoomEnabled = !scrollZoomEnabled; e.currentTarget.classList.toggle('toggle-active', scrollZoomEnabled); e.currentTarget.setAttribute('aria-pressed', String(scrollZoomEnabled)); e.currentTarget.textContent = scrollZoomEnabled ? 'Scroll zoom on' : 'Page scroll on'; setStatus(scrollZoomEnabled ? 'Scroll wheel zooms the GLB view.' : 'Scroll wheel returns to page scrolling.'); }); syncZoomControls();
  canvas.addEventListener('pointerdown', (e) => { if (document.pointerLockElement === canvas) return; e.preventDefault(); dragPointer = { id:e.pointerId, x:e.clientX, y:e.clientY }; canvas.setPointerCapture?.(e.pointerId); }); canvas.addEventListener('pointermove', (e) => { if (!dragPointer || dragPointer.id !== e.pointerId || document.pointerLockElement === canvas) return; e.preventDefault(); player.yaw -= (e.clientX - dragPointer.x) * .006; player.pitch = THREE.MathUtils.clamp(player.pitch - (e.clientY - dragPointer.y) * .005, -1.25, 1.15); dragPointer.x = e.clientX; dragPointer.y = e.clientY; }); const release = (e) => { if (dragPointer?.id === e.pointerId) dragPointer = null; }; canvas.addEventListener('pointerup', release); canvas.addEventListener('pointercancel', release); canvas.addEventListener('wheel', (e) => { if (!scrollZoomEnabled) return; e.preventDefault(); applyZoom(e.deltaY); }, { passive:false }); document.addEventListener('wheel', (e) => { if (document.pointerLockElement !== canvas || !scrollZoomEnabled) return; e.preventDefault(); applyZoom(e.deltaY); }, { passive:false });
  ...
  function resize() { const rect = stage.getBoundingClientRect(); const viewportHeight = window.visualViewport?.height || window.innerHeight || 720; const width = Math.max(320, Math.floor(Math.min(rect.width, document.documentElement.clientWidth || rect.width))); const desktopLimit = Math.max(420, viewportHeight - 180); const mobileLimit = Math.max(360, viewportHeight - 190); const limit = matchMedia('(max-width: 760px)').matches ? mobileLimit : desktopLimit; const height = Math.floor(Math.min(Math.max(360, rect.height || width * .56), limit)); renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, quality.pixelRatio)); renderer.setSize(width, height, false); camera.aspect = width / height; camera.updateProjectionMatrix(); } new ResizeObserver(resize).observe(stage); addEventListener('orientationchange', resize); window.visualViewport?.addEventListener('resize', resize); resize();
