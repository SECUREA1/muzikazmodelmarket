...
export class EnvironmentLoader {
  constructor({ scene, renderer, onProgress = () => {} }) {
    this.scene = scene; this.renderer = renderer; this.onProgress = onProgress; this.token = 0; this.world = null; this.mixers = []; this.meshes = []; this.octree = new Octree(); this.bounds = new THREE.Box3();
  ...
  async load(environment) {
    ...
    const nextWorld = new THREE.Group(); nextWorld.name = `WORLD_${environment.id}`; nextWorld.scale.setScalar(Number(environment.scale) || 1); nextWorld.rotation.set(environment.rotation.x || 0, environment.rotation.y || 0, environment.rotation.z || 0);
    const nextMixers = [];
    try {
      ...
      this.unload(); this.world = nextWorld; this.mixers = nextMixers; this.meshes = collision.visibleMeshes; this.octree = collision.octree; this.bounds = spawn.bounds; this.scene.add(nextWorld); this.onProgress(100);
