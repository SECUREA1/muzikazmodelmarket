...
export class EnvironmentLoader {
  ...
  createBuilderWorld(environment) {
    const scene = environment.builderScene;
    if (!scene || !Array.isArray(scene.objects)) throw new Error('This environment has no GLB or coded Builder scene to play.');
    const size = THREE.MathUtils.clamp(Number(scene.layoutMeta?.size || scene.size) || 40, 10, 200);
    const color = new THREE.Color(scene.layoutMeta?.groundColor || '#315b32');
    const floor = new THREE.Mesh(
      new THREE.PlaneGeometry(size, size, Math.max(1, Math.round(size / 2)), Math.max(1, Math.round(size / 2))),
      new THREE.MeshStandardMaterial({ color, roughness: .92, metalness: .02, side: THREE.DoubleSide })
    );
    floor.name = 'GROUND_BUILDER_SVG';
    floor.rotation.x = -Math.PI / 2;
    floor.receiveShadow = true;
    const root = new THREE.Group();
    root.name = `CODED_BUILDER_WORLD_${environment.id}`;
    root.userData.codedBuilderScene = true;
    root.add(floor);
    return root;
  }
  ...
  async load(environment) {
    ...
    const nextMixers = [];
    try {
      if (!urls.length) nextWorld.add(this.createBuilderWorld(environment));
