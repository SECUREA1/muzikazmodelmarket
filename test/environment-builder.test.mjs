...
test('builder lands and placed items retain verified collision floors', async () => {
  ...
  assert.match(game, /const spawn=builderRuntime\?\.worldSpawn\|\|result\.spawn;resetPlayer\(spawn\.position,spawn\.rotationY\|\|0\)/, 'map loading places the player directly at an authored or safe resolved floor spawn');
...
test('collision floors protect spawn, teleport and low-frame-rate falls', async () => {
  ...
  assert.doesNotMatch(game, /PLAYER_ENTRY_DROP_HEIGHT|dropIntoMap|entryPosition\.y \+=/, 'map entry never adds an artificial aerial drop above the verified floor');
  assert.match(game, /playerRig\.position\.copy\(alignedSpawn\)/, 'player resets use the floor-aligned position without a vertical displacement');
