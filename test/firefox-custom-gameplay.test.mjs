...
test('builder uses self-contained ESM dependencies and repository manifests', async () => {
  ...
  assert.doesNotMatch(builder, /MUZIKAZ_API|\/api\/models|\/api\/custom-maps/);
  ...
  assert.doesNotMatch(environments, /apiFetch\('\/api\/environments'|fetchGitHubGlbFiles/);
