...
export class PaymentOrderStore {
  ...
  async create(input) { return this.locked(async () => {
    ...
    const order = { orderId: randomUUID(), userId: String(input.userId || input.wallet || 'guest').slice(0, 140), wallet: String(input.wallet || '').slice(0, 140), purchaseType: String(input.purchaseType || 'GENERIC').slice(0, 60), itemId: String(input.itemId || '').slice(0, 140), quantity: Math.max(1, Number(input.quantity) || 1), metadata, fiatCurrency: 'USD', basePrice, paymentAsset: network.symbol, paymentNetwork: network.network, assetNetwork: String(input.assetNetwork || '').slice(0, 80) || null, destinationAddress: network.address, expectedAmount, transactionHash: null, amountReceived: 0, confirmations: 0, paymentStatus: 'AWAITING_PAYMENT', createdAt: now, confirmedAt: null, fulfilledAt: null };
