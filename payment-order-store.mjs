...
export class PaymentOrderStore {
  ...
  async create(input) { return this.locked(async () => {
    ...
    const order = { orderId: randomUUID(), userId: String(input.userId || input.wallet || 'guest').slice(0, 140), wallet: String(input.wallet || '').slice(0, 140), purchaseType: String(input.purchaseType || 'GENERIC').slice(0, 60), itemId: String(input.itemId || '').slice(0, 140), quantity: Math.max(1, Number(input.quantity) || 1), metadata: input.metadata && typeof input.metadata === 'object' ? input.metadata : {}, fiatCurrency: 'USD', basePrice, paymentAsset: network.symbol, paymentNetwork: network.network, assetNetwork: String(input.assetNetwork || '').slice(0, 80) || null, destinationAddress: network.address, expectedAmount, transactionHash: null, amountReceived: 0, confirmations: 0, paymentStatus: 'AWAITING_PAYMENT', createdAt: now, confirmedAt: null, fulfilledAt: null };
  ...
  async submit(id, transactionHash) { return this.locked(async () => {
    ...
    order.transactionHash = hash; order.paymentStatus = 'TRANSACTION_SUBMITTED'; await this.save(records); return this.verify(id);
  }); }
  async verify(id) {
    const records = await this.records(); const order = records.find((item) => item.orderId === id); if (!order) throw new Error('Payment order not found.');
    if (terminal.has(order.paymentStatus) || order.paymentStatus === 'PAID') return order;
    ...
    else if (result.verified && order.amountReceived >= order.expectedAmount) { order.paymentStatus = 'PAID'; order.confirmedAt = new Date().toISOString(); }
