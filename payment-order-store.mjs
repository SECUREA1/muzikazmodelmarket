...
export class PaymentOrderStore {
  ...
  async create(input) { return this.locked(async () => {
    ...
    const metadata = input.metadata && typeof input.metadata === 'object' ? { items: cleanItems(input.metadata), receiptEmail: String(input.metadata.receiptEmail || '').slice(0, 254), mzkOwner: String(input.metadata.mzkOwner || '').slice(0, 140), tokens: Math.max(0, Math.trunc(Number(input.metadata.tokens) || 0)), worldId: String(input.metadata.worldId || '').replace(/[^a-z0-9-]/gi, '').slice(0, 80), oneTime: input.metadata.oneTime === true } : { items: [] };
    const order = { orderId: randomUUID(), claimTokenHash: createHash('sha256').update(claimToken).digest('hex'), userId: String(input.userId || input.wallet || 'guest').slice(0, 140), wallet: String(input.wallet || '').slice(0, 140), purchaseType: String(input.purchaseType || 'GENERIC').slice(0, 60), itemId: String(input.itemId || '').slice(0, 140), quantity: Math.max(1, Number(input.quantity) || 1), metadata, fiatCurrency: 'USD', basePrice, paymentAsset: network.symbol, paymentNetwork: network.network, chainId: network.chainId || null, tokenAddress: network.tokenAddress || null, tokenDecimals: network.decimals, assetNetwork: String(input.assetNetwork || '').slice(0, 80) || null, destinationAddress: network.address, expectedAmount, transactionHash: null, amountReceived: 0, confirmations: 0, paymentStatus: 'AWAITING_PAYMENT', createdAt: now, confirmedAt: null, fulfilledAt: null };
